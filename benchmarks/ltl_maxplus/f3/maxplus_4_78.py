...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (F (x_0 - x_3 > -10))) U (! (x_3 - x_2 >= -11)))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "-10")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "-11"))))))
