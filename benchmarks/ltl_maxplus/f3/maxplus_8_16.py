...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_0 - x_7 >= -4)) U (X (x_4 - x_2 > -3)))
    ltl = enc.make_U(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[7]), msat_make_number(menv, "-4"))), enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[2]), msat_make_number(menv, "-3"))))
