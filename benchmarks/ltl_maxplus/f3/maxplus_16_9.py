...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (F (X (x_12 - x_5 > 13)))) U (! (x_14 - x_5 >= -20))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[12], xs[5]), msat_make_number(menv, "13"))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[14], xs[5]), msat_make_number(menv, "-20")))))
