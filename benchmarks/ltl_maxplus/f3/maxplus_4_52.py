...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_0 - x_2 > 0)) U (! (G (F (x_1 - x_0 >= -5))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[2]), msat_make_number(menv, "0"))), msat_make_not(menv, enc.make_G(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[0]), msat_make_number(menv, "-5")))))))
