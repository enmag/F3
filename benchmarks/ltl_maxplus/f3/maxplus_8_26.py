...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_7 - x_1 >= 18) | ((x_5 - x_0 >= 9) & (x_5 - x_3 >= 3)))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[1]), msat_make_number(menv, "18")), msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[0]), msat_make_number(menv, "9")), msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[3]), msat_make_number(menv, "3"))))
