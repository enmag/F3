...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_20 - x_31 > 18) | (! ((! (x_25 - x_9 > 12)) U (! (x_1 - x_22 >= -6)))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[20], xs[31]), msat_make_number(menv, "18")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[25], xs[9]), msat_make_number(menv, "12"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[22]), msat_make_number(menv, "-6"))))))
