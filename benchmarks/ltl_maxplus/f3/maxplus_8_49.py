...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_5 - x_0 > -11) & ((x_4 - x_1 >= -12) | (x_2 - x_1 >= -10)))
    ltl = msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[5], xs[0]), msat_make_number(menv, "-11")), msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[1]), msat_make_number(menv, "-12")), msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "-10"))))
