...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (x_3 - x_10 >= 19)) U (! (G (x_6 - x_7 >= -8))))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[10]), msat_make_number(menv, "19"))), msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[7]), msat_make_number(menv, "-8")))))))
