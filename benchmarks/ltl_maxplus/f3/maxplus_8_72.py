...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (x_5 - x_7 >= 7)) U (! (G (x_1 - x_5 >= -14))))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[7]), msat_make_number(menv, "7"))), msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[5]), msat_make_number(menv, "-14")))))))
