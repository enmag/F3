...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! ((x_24 - x_3 >= 0) & (x_16 - x_25 > -3))) U (! (x_6 - x_18 >= 13))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[24], xs[3]), msat_make_number(menv, "0")), msat_make_gt(menv, msat_make_minus(menv, xs[16], xs[25]), msat_make_number(menv, "-3")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[18]), msat_make_number(menv, "13")))))
