...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_1 - x_8 >= -20) & (G (x_16 - x_17 >= -2))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[8]), msat_make_number(menv, "-20")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[17]), msat_make_number(menv, "-2")))))
