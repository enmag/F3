...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_3 - x_2 >= -7) | ((x_2 - x_0 >= 15) & (x_3 - x_0 >= -9)))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "-7")), msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[0]), msat_make_number(menv, "15")), msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-9"))))
