...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_5 - x_11 >= -19) & (G (F (x_2 - x_16 >= 5))))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[11]), msat_make_number(menv, "-19")), enc.make_G(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[16]), msat_make_number(menv, "5")))))
