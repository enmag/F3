...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_19 - x_22 > 18)) U (! (! ((! (x_3 - x_21 > 14)) U (! (x_13 - x_20 >= -8)))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[19], xs[22]), msat_make_number(menv, "18"))), msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[21]), msat_make_number(menv, "14"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[13], xs[20]), msat_make_number(menv, "-8"))))))))
