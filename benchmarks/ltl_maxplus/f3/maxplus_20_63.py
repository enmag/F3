...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: ((x_10 - x_0 > 0) | (! ((! (x_4 - x_19 > 7)) U (! (x_12 - x_19 > 7)))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[10], xs[0]), msat_make_number(menv, "0")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[19]), msat_make_number(menv, "7"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[12], xs[19]), msat_make_number(menv, "7"))))))
