...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_3 - x_0 >= -2) | (! ((! (x_0 - x_3 >= 8)) U (! (x_2 - x_0 > 19)))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-2")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "8"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[0]), msat_make_number(menv, "19"))))))
