...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr0 = msat_make_plus(menv, xs[7], n_7_0)
    ...
    # ltl property: (! ((! (x_8 - x_11 > 4)) U (! ((x_7 - x_1 >= -20) | (x_2 - x_4 > -4)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[11]), msat_make_number(menv, "4"))), msat_make_not(menv, msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[1]), msat_make_number(menv, "-20")), msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[4]), msat_make_number(menv, "-4"))))))
