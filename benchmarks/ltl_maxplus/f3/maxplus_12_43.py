...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (x_3 - x_11 >= -4)) & (G (x_9 - x_6 > 0)))
    ltl = msat_make_and(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[11]), msat_make_number(menv, "-4"))), enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[9], xs[6]), msat_make_number(menv, "0"))))
