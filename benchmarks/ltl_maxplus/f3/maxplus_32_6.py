...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_19 - x_14 > -5)) U (! (X (G (x_20 - x_28 > -8))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[19], xs[14]), msat_make_number(menv, "-5"))), msat_make_not(menv, enc.make_X(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[20], xs[28]), msat_make_number(menv, "-8")))))))
