...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_2 - x_0 >= -12)) U (G (x_3 - x_1 > 13)))
    ltl = enc.make_U(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[0]), msat_make_number(menv, "-12"))), enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "13"))))
