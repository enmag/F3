...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_0 - x_4 > -11)) U (! (x_2 - x_13 >= 6))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[4]), msat_make_number(menv, "-11"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[13]), msat_make_number(menv, "6")))))))
