...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_9 - x_8 > 17) | (! ((! (x_1 - x_5 > 13)) U (! (x_10 - x_1 > 5)))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[9], xs[8]), msat_make_number(menv, "17")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[5]), msat_make_number(menv, "13"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[10], xs[1]), msat_make_number(menv, "5"))))))
