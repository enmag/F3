...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (X (x_2 - x_6 > -12))) U (! (x_0 - x_5 >= -8)))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[6]), msat_make_number(menv, "-12")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[5]), msat_make_number(menv, "-8"))))))
