...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (F (X (x_3 - x_8 >= 11)))) U (! (x_0 - x_11 >= -7))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[8]), msat_make_number(menv, "11"))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[11]), msat_make_number(menv, "-7")))))
