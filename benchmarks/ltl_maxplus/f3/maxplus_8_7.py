...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_6 - x_5 >= -17)) U (! (! ((! (x_2 - x_3 >= 7)) U (! (x_1 - x_6 >= 14)))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[5]), msat_make_number(menv, "-17"))), msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[3]), msat_make_number(menv, "7"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[6]), msat_make_number(menv, "14"))))))))
