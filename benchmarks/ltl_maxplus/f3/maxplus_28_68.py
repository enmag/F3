...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_11 - x_22 >= -2) & (F (G (x_17 - x_10 > 2))))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[11], xs[22]), msat_make_number(menv, "-2")), enc.make_F(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[17], xs[10]), msat_make_number(menv, "2")))))
