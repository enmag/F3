...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (X (F (x_25 - x_6 > -10)))) U (! (x_30 - x_13 > 10))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[25], xs[6]), msat_make_number(menv, "-10"))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[30], xs[13]), msat_make_number(menv, "10")))))
