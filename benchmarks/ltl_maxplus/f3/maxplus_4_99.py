...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_1 - x_3 >= -3) | (G (x_2 - x_1 > 7))))
    ltl = enc.make_X(msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[3]), msat_make_number(menv, "-3")), enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "7")))))
