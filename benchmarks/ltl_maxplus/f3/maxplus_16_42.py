...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! ((x_10 - x_6 >= -14) & (x_13 - x_6 > 14))) U (! (x_15 - x_2 > -16))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[10], xs[6]), msat_make_number(menv, "-14")), msat_make_gt(menv, msat_make_minus(menv, xs[13], xs[6]), msat_make_number(menv, "14")))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[15], xs[2]), msat_make_number(menv, "-16")))))
