...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_16 - x_11 >= -19) & (X (F (x_14 - x_4 > 19))))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[11]), msat_make_number(menv, "-19")), enc.make_X(enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[14], xs[4]), msat_make_number(menv, "19")))))
