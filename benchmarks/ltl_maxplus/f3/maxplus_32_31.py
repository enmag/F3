...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_31 - x_2 >= 3)) U (! ((x_1 - x_17 > -3) | (x_23 - x_29 >= 13)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[31], xs[2]), msat_make_number(menv, "3"))), msat_make_not(menv, msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[17]), msat_make_number(menv, "-3")), msat_make_geq(menv, msat_make_minus(menv, xs[23], xs[29]), msat_make_number(menv, "13"))))))
