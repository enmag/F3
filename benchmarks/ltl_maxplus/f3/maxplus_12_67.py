...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_0 - x_8 >= 5) & (X (x_3 - x_0 > -10))))
    ltl = enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[8]), msat_make_number(menv, "5")), enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-10")))))
