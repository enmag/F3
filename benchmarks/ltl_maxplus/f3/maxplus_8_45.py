...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_2 - x_0 >= -10)) U (G (x_5 - x_4 > 19)))
    ltl = enc.make_U(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[0]), msat_make_number(menv, "-10"))), enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[5], xs[4]), msat_make_number(menv, "19"))))
