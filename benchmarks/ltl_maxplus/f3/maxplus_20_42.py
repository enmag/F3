...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! ((x_18 - x_10 >= 15) & (x_13 - x_8 >= 17))) U (! (x_11 - x_18 >= 6))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[18], xs[10]), msat_make_number(menv, "15")), msat_make_geq(menv, msat_make_minus(menv, xs[13], xs[8]), msat_make_number(menv, "17")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[11], xs[18]), msat_make_number(menv, "6")))))
