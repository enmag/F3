...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (X (X (G (x_0 - x_2 >= 2)))))
    ltl = enc.make_F(enc.make_X(enc.make_X(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[2]), msat_make_number(menv, "2"))))))
