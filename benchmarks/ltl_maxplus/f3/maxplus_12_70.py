...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (x_4 - x_6 > -17)) U (! (X (x_0 - x_3 >= -20))))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[6]), msat_make_number(menv, "-17"))), msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "-20")))))))
