...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_6 - x_4 > -6)) U (! (X (G (x_1 - x_6 >= 1))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[4]), msat_make_number(menv, "-6"))), msat_make_not(menv, enc.make_X(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[6]), msat_make_number(menv, "1")))))))
