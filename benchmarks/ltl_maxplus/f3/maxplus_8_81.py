...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_1 - x_7 >= 11)) U (! (x_2 - x_5 > -16))))) U (! (x_1 - x_7 >= 11))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[7]), msat_make_number(menv, "11"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[5]), msat_make_number(menv, "-16")))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[7]), msat_make_number(menv, "11")))))
