...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (G ((x_7 - x_10 > -3) & (x_2 - x_10 >= 11))))
    ltl = enc.make_F(enc.make_G(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[10]), msat_make_number(menv, "-3")), msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[10]), msat_make_number(menv, "11")))))
