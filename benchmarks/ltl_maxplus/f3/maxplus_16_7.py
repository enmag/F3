...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_4 - x_10 > -16)) U (! (! ((! (x_5 - x_2 > 2)) U (! (x_1 - x_14 > -10)))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[10]), msat_make_number(menv, "-16"))), msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[5], xs[2]), msat_make_number(menv, "2"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[14]), msat_make_number(menv, "-10"))))))))
