...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (x_19 - x_14 > 2)) & (G (x_26 - x_10 >= 18)))
    ltl = msat_make_and(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[19], xs[14]), msat_make_number(menv, "2"))), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[26], xs[10]), msat_make_number(menv, "18"))))
