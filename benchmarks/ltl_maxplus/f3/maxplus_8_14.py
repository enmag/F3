...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((G (x_6 - x_5 > 0)) U (x_0 - x_3 >= 16)))
    ltl = enc.make_X(enc.make_U(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[5]), msat_make_number(menv, "0"))), msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "16"))))
