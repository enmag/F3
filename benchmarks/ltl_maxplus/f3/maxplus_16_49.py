...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_12 - x_1 > 19) & ((x_7 - x_3 >= 17) | (x_6 - x_3 > 1)))
    ltl = msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[12], xs[1]), msat_make_number(menv, "19")), msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[3]), msat_make_number(menv, "17")), msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[3]), msat_make_number(menv, "1"))))
