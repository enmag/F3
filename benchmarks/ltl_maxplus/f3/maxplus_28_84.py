...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_19 - x_11 > -16) | (! ((! (x_14 - x_15 >= -17)) U (! (x_2 - x_9 > -11)))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[19], xs[11]), msat_make_number(menv, "-16")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[14], xs[15]), msat_make_number(menv, "-17"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[9]), msat_make_number(menv, "-11"))))))
