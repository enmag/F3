...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_7 - x_9 >= -11) & ((x_0 - x_4 >= -17) | (x_10 - x_5 >= -4)))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[9]), msat_make_number(menv, "-11")), msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[4]), msat_make_number(menv, "-17")), msat_make_geq(menv, msat_make_minus(menv, xs[10], xs[5]), msat_make_number(menv, "-4"))))
