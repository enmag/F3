...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (F (X (x_1 - x_14 > -17)))) U (! (x_7 - x_1 >= -1))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[14]), msat_make_number(menv, "-17"))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[1]), msat_make_number(menv, "-1")))))
