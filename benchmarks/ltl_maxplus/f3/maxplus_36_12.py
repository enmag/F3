...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_0 - x_20 > 12)) U (! (x_9 - x_34 > -5))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[20]), msat_make_number(menv, "12"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[9], xs[34]), msat_make_number(menv, "-5")))))))
