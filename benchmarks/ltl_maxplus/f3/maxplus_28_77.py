...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_18 - x_7 >= -6) & (G (x_16 - x_10 >= -1))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[18], xs[7]), msat_make_number(menv, "-6")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[10]), msat_make_number(menv, "-1")))))
