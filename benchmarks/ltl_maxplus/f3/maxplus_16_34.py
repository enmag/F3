...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (X (x_4 - x_12 > -19))) U (! (x_8 - x_3 > -8)))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[12]), msat_make_number(menv, "-19")))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[3]), msat_make_number(menv, "-8"))))))
