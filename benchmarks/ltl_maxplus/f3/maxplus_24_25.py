...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_23 - x_20 >= -5)) & (F (x_10 - x_19 > 15)))
    ltl = msat_make_and(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[23], xs[20]), msat_make_number(menv, "-5"))), enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[10], xs[19]), msat_make_number(menv, "15"))))
