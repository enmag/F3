...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_5 - x_7 > 3) & (F (G (x_4 - x_5 > -3))))
    ltl = msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[5], xs[7]), msat_make_number(menv, "3")), enc.make_F(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[5]), msat_make_number(menv, "-3")))))
