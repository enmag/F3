...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((F (x_3 - x_2 >= 11)) U (G (x_1 - x_0 >= -13)))
    ltl = enc.make_U(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "11"))), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[0]), msat_make_number(menv, "-13"))))
