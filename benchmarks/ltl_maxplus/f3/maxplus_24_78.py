...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (F (x_16 - x_20 > -1))) U (! (x_12 - x_9 >= -6)))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[16], xs[20]), msat_make_number(menv, "-1")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[9]), msat_make_number(menv, "-6"))))))
