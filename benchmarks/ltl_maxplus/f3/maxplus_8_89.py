...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (x_0 - x_6 > -15)) U (! (X (x_2 - x_6 > 8))))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[6]), msat_make_number(menv, "-15"))), msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[6]), msat_make_number(menv, "8")))))))
