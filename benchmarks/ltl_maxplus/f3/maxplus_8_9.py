...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (F (X (x_1 - x_7 >= -6)))) U (! (x_5 - x_1 >= 10))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[7]), msat_make_number(menv, "-6"))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[1]), msat_make_number(menv, "10")))))
