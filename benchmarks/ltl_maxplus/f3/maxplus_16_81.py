...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_5 - x_1 >= 4)) U (! (x_8 - x_6 >= -3))))) U (! (x_5 - x_1 >= 4))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[1]), msat_make_number(menv, "4"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[8], xs[6]), msat_make_number(menv, "-3")))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[1]), msat_make_number(menv, "4")))))
