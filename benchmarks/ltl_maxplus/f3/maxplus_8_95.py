...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (((x_2 - x_3 >= -7) | (x_2 - x_1 > -3)) U (x_0 - x_5 >= 16))
    ltl = enc.make_U(msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[3]), msat_make_number(menv, "-7")), msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "-3"))), msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[5]), msat_make_number(menv, "16")))
