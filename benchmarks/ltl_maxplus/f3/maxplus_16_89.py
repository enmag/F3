...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (x_13 - x_0 >= 4)) U (! (X (x_4 - x_13 >= 0))))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[13], xs[0]), msat_make_number(menv, "4"))), msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[13]), msat_make_number(menv, "0")))))))
