...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_8 - x_18 > -4) | (X (F (x_16 - x_11 >= -1))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[18]), msat_make_number(menv, "-4")), enc.make_X(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[11]), msat_make_number(menv, "-1")))))
