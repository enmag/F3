...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (F (X (x_3 - x_0 >= -14)))) U (! (x_2 - x_0 >= -20))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-14"))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[0]), msat_make_number(menv, "-20")))))
