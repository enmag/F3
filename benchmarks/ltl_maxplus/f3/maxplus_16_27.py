...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_6 - x_15 >= -7)) U (G (x_14 - x_12 >= -13)))
    ltl = enc.make_U(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[15]), msat_make_number(menv, "-7"))), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[14], xs[12]), msat_make_number(menv, "-13"))))
