...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_16 - x_24 > -11) & (G (x_0 - x_9 >= -8))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[16], xs[24]), msat_make_number(menv, "-11")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[9]), msat_make_number(menv, "-8")))))
