...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (X (x_7 - x_4 >= -6))) U (! (F (x_11 - x_0 >= 4)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[4]), msat_make_number(menv, "-6")))), msat_make_not(menv, enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[11], xs[0]), msat_make_number(menv, "4"))))))
