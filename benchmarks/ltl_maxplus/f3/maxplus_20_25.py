...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_6 - x_1 >= 5)) & (F (x_12 - x_8 >= -8)))
    ltl = msat_make_and(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[1]), msat_make_number(menv, "5"))), enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[8]), msat_make_number(menv, "-8"))))
