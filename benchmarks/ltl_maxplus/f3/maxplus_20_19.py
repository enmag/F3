...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_2 - x_5 > -10)) U (! (x_1 - x_15 >= -6))))) U (! (x_14 - x_11 >= 18))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[5]), msat_make_number(menv, "-10"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[15]), msat_make_number(menv, "-6")))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[14], xs[11]), msat_make_number(menv, "18")))))
