...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (G ((x_2 - x_1 >= -8) & (x_0 - x_4 > 16))))
    ltl = enc.make_F(enc.make_G(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "-8")), msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[4]), msat_make_number(menv, "16")))))
