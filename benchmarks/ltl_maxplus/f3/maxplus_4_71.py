...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_2 - x_1 > -8) & (G (F (x_3 - x_0 > 19))))
    ltl = msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "-8")), enc.make_G(enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "19")))))
