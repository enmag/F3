...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr14 = msat_make_plus(menv, xs[20], n_4_0)
    ...
    # ltl property: (! ((! (X (x_1 - x_26 >= 20))) U (! (F (x_11 - x_23 > 3)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[26]), msat_make_number(menv, "20")))), msat_make_not(menv, enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[23]), msat_make_number(menv, "3"))))))
