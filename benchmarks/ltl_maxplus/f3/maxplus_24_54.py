...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (F (x_6 - x_1 >= 12))) U (! (x_6 - x_1 >= 12)))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[1]), msat_make_number(menv, "12")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[1]), msat_make_number(menv, "12"))))))
