...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_2 - x_11 >= -10)) U (! (x_17 - x_20 >= 20))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[11]), msat_make_number(menv, "-10"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[17], xs[20]), msat_make_number(menv, "20")))))))
