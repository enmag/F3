...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_9 - x_7 >= -8)) U (! (X (G (x_6 - x_12 > -4))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[7]), msat_make_number(menv, "-8"))), msat_make_not(menv, enc.make_X(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[12]), msat_make_number(menv, "-4")))))))
