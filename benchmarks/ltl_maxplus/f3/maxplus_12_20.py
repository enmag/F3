...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_0 - x_6 >= 7) | (X (F (x_9 - x_8 >= 9))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[6]), msat_make_number(menv, "7")), enc.make_X(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[8]), msat_make_number(menv, "9")))))
