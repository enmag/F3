...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (F (X (x_15 - x_7 >= -13)))) U (! (x_15 - x_3 >= 16))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[15], xs[7]), msat_make_number(menv, "-13"))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[15], xs[3]), msat_make_number(menv, "16")))))
