...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_2 - x_23 >= -1) | (! ((! (x_9 - x_4 >= 9)) U (! (x_16 - x_11 >= -14)))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[23]), msat_make_number(menv, "-1")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[4]), msat_make_number(menv, "9"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[11]), msat_make_number(menv, "-14"))))))
