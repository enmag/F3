...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_7 - x_1 > 10) & (G (x_4 - x_3 >= 9))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[1]), msat_make_number(menv, "10")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[3]), msat_make_number(menv, "9")))))
