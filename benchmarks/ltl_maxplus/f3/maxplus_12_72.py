...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (x_9 - x_8 > 19)) U (! (G (x_3 - x_11 >= -10))))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[9], xs[8]), msat_make_number(menv, "19"))), msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[11]), msat_make_number(menv, "-10")))))))
