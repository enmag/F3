...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_5 - x_6 >= -2) | (X (G (x_7 - x_6 >= -3))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[6]), msat_make_number(menv, "-2")), enc.make_X(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[6]), msat_make_number(menv, "-3")))))
