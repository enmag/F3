...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (x_0 - x_6 > -2)) U (! (G (x_7 - x_6 >= 9))))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[6]), msat_make_number(menv, "-2"))), msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[6]), msat_make_number(menv, "9")))))))
