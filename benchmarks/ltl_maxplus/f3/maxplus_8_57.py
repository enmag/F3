...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (X (x_7 - x_1 >= -20))) U (! (x_7 - x_0 >= 2)))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[1]), msat_make_number(menv, "-20")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[0]), msat_make_number(menv, "2"))))))
