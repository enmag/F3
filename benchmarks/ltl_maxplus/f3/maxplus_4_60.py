...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G ((x_1 - x_0 >= -4) | (X (x_3 - x_0 > -9))))
    ltl = enc.make_G(msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[0]), msat_make_number(menv, "-4")), enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-9")))))
