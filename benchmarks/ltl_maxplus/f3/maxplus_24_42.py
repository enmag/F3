...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! ((x_15 - x_3 >= -8) & (x_21 - x_19 >= 0))) U (! (x_4 - x_1 >= -7))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[15], xs[3]), msat_make_number(menv, "-8")), msat_make_geq(menv, msat_make_minus(menv, xs[21], xs[19]), msat_make_number(menv, "0")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[1]), msat_make_number(menv, "-7")))))
