...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_3 - x_2 >= 14) | (X (F (x_2 - x_1 >= -1))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "14")), enc.make_X(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "-1")))))
