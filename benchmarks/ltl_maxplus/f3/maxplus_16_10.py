...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (X (! ((! (x_7 - x_6 > 7)) U (! (x_9 - x_14 > -8))))))
    ltl = enc.make_X(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[6]), msat_make_number(menv, "7"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[9], xs[14]), msat_make_number(menv, "-8")))))))
