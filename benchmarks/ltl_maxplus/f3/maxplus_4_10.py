...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (X (! ((! (x_0 - x_1 >= 6)) U (! (x_1 - x_2 >= 12))))))
    ltl = enc.make_X(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[1]), msat_make_number(menv, "6"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[2]), msat_make_number(menv, "12")))))))
