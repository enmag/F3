...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_19 - x_0 >= 8)) & (F (x_11 - x_0 > 10)))
    ltl = msat_make_and(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[19], xs[0]), msat_make_number(menv, "8"))), enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[0]), msat_make_number(menv, "10"))))
