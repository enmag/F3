...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (X (! ((! (x_9 - x_18 >= 7)) U (! (x_8 - x_16 > 20))))))
    ltl = enc.make_X(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[18]), msat_make_number(menv, "7"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[16]), msat_make_number(menv, "20")))))))
