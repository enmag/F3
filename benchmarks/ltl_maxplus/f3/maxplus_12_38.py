...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_1 - x_0 >= 0)) U (! (x_0 - x_1 >= -4))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[0]), msat_make_number(menv, "0"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[1]), msat_make_number(menv, "-4")))))))
