...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_4 - x_7 > 0)) U (X (x_4 - x_5 >= -20)))
    ltl = enc.make_U(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[7]), msat_make_number(menv, "0"))), enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[5]), msat_make_number(menv, "-20"))))
