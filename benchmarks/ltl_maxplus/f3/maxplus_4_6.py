...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_0 - x_1 >= -3)) U (! (X (G (x_1 - x_0 >= -12))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[1]), msat_make_number(menv, "-3"))), msat_make_not(menv, enc.make_X(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[0]), msat_make_number(menv, "-12")))))))
