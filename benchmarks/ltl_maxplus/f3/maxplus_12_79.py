...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_11 - x_6 > 6) & (G (x_10 - x_6 >= -3))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[6]), msat_make_number(menv, "6")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[10], xs[6]), msat_make_number(menv, "-3")))))
