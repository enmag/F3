...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_9 - x_3 >= -15)) U (! (X (X (x_8 - x_10 >= 10))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[3]), msat_make_number(menv, "-15"))), msat_make_not(menv, enc.make_X(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[8], xs[10]), msat_make_number(menv, "10")))))))
