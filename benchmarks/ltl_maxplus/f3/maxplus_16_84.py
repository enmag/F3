...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: ((x_6 - x_3 > 1) | (! ((! (x_1 - x_8 >= 13)) U (! (x_5 - x_3 > -9)))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[3]), msat_make_number(menv, "1")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[8]), msat_make_number(menv, "13"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[5], xs[3]), msat_make_number(menv, "-9"))))))
