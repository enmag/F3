...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr0 = msat_make_plus(menv, xs[6], n_19_0)
    ...
    expr19 = msat_make_plus(menv, xs[32], n_15_0)
    _t = msat_make_and(menv,
    ...
    # ltl property: (X (X (! ((! (x_33 - x_17 > 12)) U (! (x_5 - x_18 >= 8))))))
    ltl = enc.make_X(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[33], xs[17]), msat_make_number(menv, "12"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[18]), msat_make_number(menv, "8")))))))
