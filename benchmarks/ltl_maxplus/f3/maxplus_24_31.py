...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_10 - x_6 > 17)) U (! ((x_22 - x_3 >= -1) | (x_9 - x_18 >= -16)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[10], xs[6]), msat_make_number(menv, "17"))), msat_make_not(menv, msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[22], xs[3]), msat_make_number(menv, "-1")), msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[18]), msat_make_number(menv, "-16"))))))
