...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_16 - x_0 > 7) & (X (F (x_11 - x_6 >= 10))))
    ltl = msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[16], xs[0]), msat_make_number(menv, "7")), enc.make_X(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[11], xs[6]), msat_make_number(menv, "10")))))
