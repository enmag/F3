...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_2 - x_4 >= 3) & (G (x_2 - x_1 > -12))))
    ltl = enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[4]), msat_make_number(menv, "3")), enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "-12")))))
