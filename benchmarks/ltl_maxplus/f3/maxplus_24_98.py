...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr1 = msat_make_plus(menv, xs[9], n_2_0)
    ...
    # ltl property: (! ((! (X (x_19 - x_11 >= -13))) U (! (F (x_11 - x_10 > 20)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[19], xs[11]), msat_make_number(menv, "-13")))), msat_make_not(menv, enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[10]), msat_make_number(menv, "20"))))))
