...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_3 - x_7 >= -15) | ((x_0 - x_4 >= 0) & (x_5 - x_6 >= 8)))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[7]), msat_make_number(menv, "-15")), msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[4]), msat_make_number(menv, "0")), msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[6]), msat_make_number(menv, "8"))))
