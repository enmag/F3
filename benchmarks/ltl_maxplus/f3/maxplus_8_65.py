...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G ((x_0 - x_5 >= -17) | (G (x_5 - x_7 >= -20))))
    ltl = enc.make_G(msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[5]), msat_make_number(menv, "-17")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[7]), msat_make_number(menv, "-20")))))
