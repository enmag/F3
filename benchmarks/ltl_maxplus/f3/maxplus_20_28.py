...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_3 - x_13 >= 2) & (G (x_14 - x_0 >= -16))))
    ltl = enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[13]), msat_make_number(menv, "2")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[14], xs[0]), msat_make_number(menv, "-16")))))
