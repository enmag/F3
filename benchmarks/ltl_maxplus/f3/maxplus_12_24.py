...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_1 - x_8 > 9) | ((x_7 - x_10 >= 10) & (x_0 - x_4 > 1)))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[8]), msat_make_number(menv, "9")), msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[10]), msat_make_number(menv, "10")), msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[4]), msat_make_number(menv, "1"))))
