...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (x_19 - x_3 >= -6)) & (G (x_13 - x_15 >= 8)))
    ltl = msat_make_and(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[19], xs[3]), msat_make_number(menv, "-6"))), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[13], xs[15]), msat_make_number(menv, "8"))))
