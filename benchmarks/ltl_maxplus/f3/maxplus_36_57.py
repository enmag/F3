...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr5 = msat_make_plus(menv, xs[18], n_19_0)
    ...
    # ltl property: (F (! ((! (X (x_3 - x_10 > 9))) U (! (x_26 - x_2 > -6)))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[10]), msat_make_number(menv, "9")))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[26], xs[2]), msat_make_number(menv, "-6"))))))
