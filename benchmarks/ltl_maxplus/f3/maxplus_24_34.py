...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (X (x_8 - x_5 > 16))) U (! (x_16 - x_23 >= -19)))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[5]), msat_make_number(menv, "16")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[23]), msat_make_number(menv, "-19"))))))
