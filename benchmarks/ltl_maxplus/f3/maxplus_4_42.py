...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! ((x_0 - x_2 >= 11) & (x_1 - x_3 > 4))) U (! (x_3 - x_0 >= -18))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[2]), msat_make_number(menv, "11")), msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[3]), msat_make_number(menv, "4")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-18")))))
