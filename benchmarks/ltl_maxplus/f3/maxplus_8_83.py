...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_0 - x_6 >= 8) & ((x_6 - x_0 > -17) U (x_2 - x_3 >= 16)))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[6]), msat_make_number(menv, "8")), enc.make_U(msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[0]), msat_make_number(menv, "-17")), msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[3]), msat_make_number(menv, "16"))))
