...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_11 - x_1 >= -4) & (F (G (x_3 - x_4 >= -12))))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[11], xs[1]), msat_make_number(menv, "-4")), enc.make_F(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[4]), msat_make_number(menv, "-12")))))
