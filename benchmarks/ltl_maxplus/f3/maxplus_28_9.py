...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (F (X (x_26 - x_5 > -9)))) U (! (x_15 - x_26 > -14))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[26], xs[5]), msat_make_number(menv, "-9"))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[15], xs[26]), msat_make_number(menv, "-14")))))
