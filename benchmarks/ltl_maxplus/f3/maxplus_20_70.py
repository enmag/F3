...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (x_3 - x_14 > -18)) U (! (X (x_11 - x_3 >= 20))))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[14]), msat_make_number(menv, "-18"))), msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[11], xs[3]), msat_make_number(menv, "20")))))))
