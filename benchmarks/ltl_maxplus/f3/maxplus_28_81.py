...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_12 - x_24 >= -3)) U (! (x_5 - x_19 >= 3))))) U (! (x_12 - x_24 >= -3))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[24]), msat_make_number(menv, "-3"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[19]), msat_make_number(menv, "3")))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[24]), msat_make_number(menv, "-3")))))
