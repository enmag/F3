...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr17 = msat_make_plus(menv, xs[27], n_3_0)
    _t = msat_make_and(menv,
    ...
    expr2 = msat_make_plus(menv, xs[13], n_14_0)
    ...
    # ltl property: (F (! ((! (x_14 - x_17 > 0)) U (! (G (x_5 - x_22 >= -19))))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[14], xs[17]), msat_make_number(menv, "0"))), msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[22]), msat_make_number(menv, "-19")))))))
