...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (X (x_16 - x_8 > -11))) U (x_4 - x_19 >= -3))
    ltl = enc.make_U(enc.make_X(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[16], xs[8]), msat_make_number(menv, "-11")))), msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[19]), msat_make_number(menv, "-3")))
