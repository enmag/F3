...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_33 - x_34 >= -5) & (F (G (x_26 - x_34 >= -9))))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[33], xs[34]), msat_make_number(menv, "-5")), enc.make_F(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[26], xs[34]), msat_make_number(menv, "-9")))))
