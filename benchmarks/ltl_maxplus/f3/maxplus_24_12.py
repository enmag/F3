...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_23 - x_12 >= 8)) U (! (x_12 - x_17 >= 20))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[23], xs[12]), msat_make_number(menv, "8"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[17]), msat_make_number(menv, "20")))))))
