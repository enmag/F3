...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_19 - x_9 >= -18)) U (! (X (X (x_16 - x_19 >= 8))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[19], xs[9]), msat_make_number(menv, "-18"))), msat_make_not(menv, enc.make_X(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[19]), msat_make_number(menv, "8")))))))
