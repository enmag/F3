...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_3 - x_0 > -9) & (G (x_2 - x_0 >= -18))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-9")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[0]), msat_make_number(menv, "-18")))))
