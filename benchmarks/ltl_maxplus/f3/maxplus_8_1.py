...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_6 - x_0 > 10) | (X (G (x_3 - x_2 >= 15))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[0]), msat_make_number(menv, "10")), enc.make_X(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "15")))))
