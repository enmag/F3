...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((F (x_1 - x_5 >= -19)) & (X (x_0 - x_3 >= 16)))
    ltl = msat_make_and(menv, enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[5]), msat_make_number(menv, "-19"))), enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "16"))))
