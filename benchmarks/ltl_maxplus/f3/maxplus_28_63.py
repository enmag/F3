...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: ((x_16 - x_8 >= -1) | (! ((! (x_12 - x_19 >= 13)) U (! (x_0 - x_11 > -13)))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[8]), msat_make_number(menv, "-1")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[19]), msat_make_number(menv, "13"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[11]), msat_make_number(menv, "-13"))))))
