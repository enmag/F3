...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr1 = msat_make_plus(menv, xs[8], n_4_0)
    ...
    expr11 = msat_make_plus(menv, xs[16], n_8_0)
    ...
    # ltl property: (G (! ((! (X (x_7 - x_22 >= -2))) U (! (x_15 - x_17 >= 3)))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[22]), msat_make_number(menv, "-2")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[15], xs[17]), msat_make_number(menv, "3"))))))
