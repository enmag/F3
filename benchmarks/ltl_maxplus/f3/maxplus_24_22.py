...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G ((G (x_23 - x_8 >= 8)) U (x_23 - x_1 > -13)))
    ltl = enc.make_G(enc.make_U(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[23], xs[8]), msat_make_number(menv, "8"))), msat_make_gt(menv, msat_make_minus(menv, xs[23], xs[1]), msat_make_number(menv, "-13"))))
