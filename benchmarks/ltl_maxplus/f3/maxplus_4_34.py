...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (X (x_3 - x_0 > 13))) U (! (x_3 - x_1 >= -11)))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "13")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "-11"))))))
