...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (X (x_1 - x_6 >= 12))) U (x_2 - x_9 > -3))
    ltl = enc.make_U(enc.make_X(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[6]), msat_make_number(menv, "12")))), msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[9]), msat_make_number(menv, "-3")))
