...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    expr10 = msat_make_plus(menv, xs[13], n_16_0)
    ...
    # ltl property: (G (X (! ((! (x_28 - x_12 >= -13)) U (! (x_22 - x_21 > 7))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[28], xs[12]), msat_make_number(menv, "-13"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[22], xs[21]), msat_make_number(menv, "7")))))))
