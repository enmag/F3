...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (F (x_27 - x_22 > 2))) U (! (x_9 - x_4 > -12)))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[27], xs[22]), msat_make_number(menv, "2")))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[9], xs[4]), msat_make_number(menv, "-12"))))))
