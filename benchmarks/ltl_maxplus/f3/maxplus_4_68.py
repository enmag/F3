...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: ((x_0 - x_3 >= -6) & (F (G (x_2 - x_1 > 6))))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "-6")), enc.make_F(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "6")))))
