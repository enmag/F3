...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (x_3 - x_13 > -4)) U (! (G (x_8 - x_3 >= 15))))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[13]), msat_make_number(menv, "-4"))), msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[8], xs[3]), msat_make_number(menv, "15")))))))
