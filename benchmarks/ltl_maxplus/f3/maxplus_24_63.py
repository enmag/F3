...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: ((x_20 - x_17 >= 17) | (! ((! (x_20 - x_10 > -20)) U (! (x_8 - x_13 >= 1)))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[20], xs[17]), msat_make_number(menv, "17")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[20], xs[10]), msat_make_number(menv, "-20"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[8], xs[13]), msat_make_number(menv, "1"))))))
