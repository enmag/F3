...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_24 - x_11 >= -17)) U (! (G (F (x_0 - x_24 >= 7))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[24], xs[11]), msat_make_number(menv, "-17"))), msat_make_not(menv, enc.make_G(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[24]), msat_make_number(menv, "7")))))))
