...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_4 - x_14 > 13)) U (! (x_13 - x_5 >= -18))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[14]), msat_make_number(menv, "13"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[13], xs[5]), msat_make_number(menv, "-18")))))))
