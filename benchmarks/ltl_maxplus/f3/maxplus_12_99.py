...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_3 - x_2 >= 15) | (G (x_1 - x_4 >= 17))))
    ltl = enc.make_X(msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "15")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[4]), msat_make_number(menv, "17")))))
