...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (X ((x_6 - x_26 >= 3) & (x_2 - x_13 > -4))))
    ltl = enc.make_F(enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[26]), msat_make_number(menv, "3")), msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[13]), msat_make_number(menv, "-4")))))
