...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((F (x_2 - x_1 > 8)) U (x_0 - x_2 >= 4)))
    ltl = enc.make_F(enc.make_U(enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "8"))), msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[2]), msat_make_number(menv, "4"))))
