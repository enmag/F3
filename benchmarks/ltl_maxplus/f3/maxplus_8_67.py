...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_2 - x_1 >= -8) & (X (x_1 - x_5 >= -15))))
    ltl = enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "-8")), enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[5]), msat_make_number(menv, "-15")))))
