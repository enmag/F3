...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (G (x_10 - x_7 >= 0))) U (! (X (x_11 - x_2 >= 17)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[10], xs[7]), msat_make_number(menv, "0")))), msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[11], xs[2]), msat_make_number(menv, "17"))))))
