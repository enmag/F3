...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_0 - x_3 >= -5) & (G (x_3 - x_1 >= -7))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "-5")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "-7")))))
