...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (G (x_12 - x_7 > 4))) U (! (X (x_15 - x_6 >= 10)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[12], xs[7]), msat_make_number(menv, "4")))), msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[15], xs[6]), msat_make_number(menv, "10"))))))
