...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_12 - x_11 >= -3)) U (! (X (G (x_27 - x_26 >= 10))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[11]), msat_make_number(menv, "-3"))), msat_make_not(menv, enc.make_X(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[27], xs[26]), msat_make_number(menv, "10")))))))
