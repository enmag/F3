...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_13 - x_6 > -9)) U (! ((x_21 - x_24 >= 14) | (x_17 - x_24 > 4)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[13], xs[6]), msat_make_number(menv, "-9"))), msat_make_not(menv, msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[21], xs[24]), msat_make_number(menv, "14")), msat_make_gt(menv, msat_make_minus(menv, xs[17], xs[24]), msat_make_number(menv, "4"))))))
