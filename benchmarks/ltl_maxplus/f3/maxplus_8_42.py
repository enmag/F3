...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! ((x_1 - x_4 > 8) & (x_3 - x_0 > -16))) U (! (x_4 - x_7 >= -11))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[4]), msat_make_number(menv, "8")), msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-16")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[7]), msat_make_number(menv, "-11")))))
