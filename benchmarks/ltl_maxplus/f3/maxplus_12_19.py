...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_6 - x_3 > 2)) U (! (x_1 - x_10 >= -2))))) U (! (x_11 - x_5 > -2))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[3]), msat_make_number(menv, "2"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[10]), msat_make_number(menv, "-2")))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[5]), msat_make_number(menv, "-2")))))
