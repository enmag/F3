...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (x_2 - x_7 > -13)) | (F (x_5 - x_1 >= -2)))
    ltl = msat_make_or(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[7]), msat_make_number(menv, "-13"))), enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[1]), msat_make_number(menv, "-2"))))
