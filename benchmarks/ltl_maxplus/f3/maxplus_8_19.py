...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_7 - x_3 >= -6)) U (! (x_3 - x_7 >= 0))))) U (! (x_1 - x_4 > -16))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[3]), msat_make_number(menv, "-6"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[7]), msat_make_number(menv, "0")))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[4]), msat_make_number(menv, "-16")))))
