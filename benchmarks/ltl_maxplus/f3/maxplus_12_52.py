...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_8 - x_3 > -11)) U (! (G (F (x_0 - x_5 >= -4))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[3]), msat_make_number(menv, "-11"))), msat_make_not(menv, enc.make_G(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[5]), msat_make_number(menv, "-4")))))))
