...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (G ((x_0 - x_3 > 16) & (x_1 - x_2 >= -7))))
    ltl = enc.make_F(enc.make_G(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "16")), msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[2]), msat_make_number(menv, "-7")))))
