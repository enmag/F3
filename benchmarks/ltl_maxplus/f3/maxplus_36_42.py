...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! ((x_10 - x_5 > 1) & (x_6 - x_32 > 16))) U (! (x_25 - x_17 >= -12))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[10], xs[5]), msat_make_number(menv, "1")), msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[32]), msat_make_number(menv, "16")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[25], xs[17]), msat_make_number(menv, "-12")))))
