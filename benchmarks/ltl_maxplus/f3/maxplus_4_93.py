...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (X (F (G (x_0 - x_1 > 3)))))
    ltl = enc.make_F(enc.make_X(enc.make_F(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[1]), msat_make_number(menv, "3"))))))
