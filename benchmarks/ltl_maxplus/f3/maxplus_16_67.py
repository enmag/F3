...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_7 - x_4 >= 8) & (X (x_14 - x_2 > -4))))
    ltl = enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[4]), msat_make_number(menv, "8")), enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[14], xs[2]), msat_make_number(menv, "-4")))))
