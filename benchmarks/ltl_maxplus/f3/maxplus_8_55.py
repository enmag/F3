...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_1 - x_2 > 10)) U (! (X (X (x_6 - x_3 > 19))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[2]), msat_make_number(menv, "10"))), msat_make_not(menv, enc.make_X(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[3]), msat_make_number(menv, "19")))))))
