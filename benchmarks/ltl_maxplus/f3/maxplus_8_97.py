...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G ((X (x_5 - x_7 > 8)) U (x_1 - x_6 > -18)))
    ltl = enc.make_G(enc.make_U(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[5], xs[7]), msat_make_number(menv, "8"))), msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[6]), msat_make_number(menv, "-18"))))
