...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (x_2 - x_3 > 1)) & (G (x_0 - x_2 >= 2)))
    ltl = msat_make_and(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[3]), msat_make_number(menv, "1"))), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[2]), msat_make_number(menv, "2"))))
