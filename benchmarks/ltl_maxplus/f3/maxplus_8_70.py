...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (x_5 - x_2 > 1)) U (! (X (x_5 - x_6 > -13))))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[5], xs[2]), msat_make_number(menv, "1"))), msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[5], xs[6]), msat_make_number(menv, "-13")))))))
