...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (X (x_29 - x_18 > -5))) U (! (F (x_27 - x_17 >= -4)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[29], xs[18]), msat_make_number(menv, "-5")))), msat_make_not(menv, enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[27], xs[17]), msat_make_number(menv, "-4"))))))
