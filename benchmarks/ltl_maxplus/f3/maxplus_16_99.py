...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_8 - x_1 > -18) | (G (x_11 - x_2 > 0))))
    ltl = enc.make_X(msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[1]), msat_make_number(menv, "-18")), enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[2]), msat_make_number(menv, "0")))))
