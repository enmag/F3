...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_16 - x_20 >= 1)) U (! (x_15 - x_17 > 2))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[20]), msat_make_number(menv, "1"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[15], xs[17]), msat_make_number(menv, "2")))))))
