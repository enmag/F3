...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_15 - x_7 >= 2) | ((x_6 - x_12 >= 15) & (x_3 - x_4 >= 14)))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[15], xs[7]), msat_make_number(menv, "2")), msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[12]), msat_make_number(menv, "15")), msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[4]), msat_make_number(menv, "14"))))
