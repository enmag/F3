...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (X (F (x_15 - x_18 > -1)))) U (! (x_9 - x_6 > 5))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[15], xs[18]), msat_make_number(menv, "-1"))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[9], xs[6]), msat_make_number(menv, "5")))))
