...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (x_23 - x_0 >= -7)) U (! (G (x_22 - x_1 >= 12))))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[23], xs[0]), msat_make_number(menv, "-7"))), msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[22], xs[1]), msat_make_number(menv, "12")))))))
