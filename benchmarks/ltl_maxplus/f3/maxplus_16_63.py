...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_12 - x_11 >= -18) | (! ((! (x_8 - x_14 > 9)) U (! (x_7 - x_6 >= -17)))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[11]), msat_make_number(menv, "-18")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[14]), msat_make_number(menv, "9"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[6]), msat_make_number(menv, "-17"))))))
