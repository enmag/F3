...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_11 - x_6 > -18)) U (! (x_2 - x_11 > -1))))) U (! (x_18 - x_16 >= -5))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[6]), msat_make_number(menv, "-18"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[11]), msat_make_number(menv, "-1")))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[18], xs[16]), msat_make_number(menv, "-5")))))
