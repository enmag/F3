...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_2 - x_1 >= 4) & (G (F (x_2 - x_1 >= 9))))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "4")), enc.make_G(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "9")))))
