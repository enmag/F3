...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (X (X (! ((! (x_22 - x_5 > -13)) U (! (x_16 - x_18 > 0))))))
    ltl = enc.make_X(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[22], xs[5]), msat_make_number(menv, "-13"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[16], xs[18]), msat_make_number(menv, "0")))))))
