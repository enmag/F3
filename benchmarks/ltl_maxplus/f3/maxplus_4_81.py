...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_3 - x_1 > 11)) U (! (x_1 - x_3 >= 18))))) U (! (x_3 - x_1 > 11))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "11"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[3]), msat_make_number(menv, "18")))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "11")))))
