...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (X (F (x_2 - x_1 >= 4)))) U (! (x_1 - x_0 > -20))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "4"))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[0]), msat_make_number(menv, "-20")))))
