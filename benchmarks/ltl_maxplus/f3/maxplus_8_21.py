...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (G ((x_3 - x_2 > -14) & (x_6 - x_7 >= 12))))
    ltl = enc.make_F(enc.make_G(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "-14")), msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[7]), msat_make_number(menv, "12")))))
