...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G ((G (x_8 - x_7 > -1)) U (x_10 - x_2 >= 6)))
    ltl = enc.make_G(enc.make_U(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[7]), msat_make_number(menv, "-1"))), msat_make_geq(menv, msat_make_minus(menv, xs[10], xs[2]), msat_make_number(menv, "6"))))
