...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (X (x_19 - x_7 >= -2))) U (x_14 - x_13 >= -9))
    ltl = enc.make_U(enc.make_X(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[19], xs[7]), msat_make_number(menv, "-2")))), msat_make_geq(menv, msat_make_minus(menv, xs[14], xs[13]), msat_make_number(menv, "-9")))
