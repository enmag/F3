...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((F (x_2 - x_17 >= 10)) & (X (x_16 - x_4 >= -10)))
    ltl = msat_make_and(menv, enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[17]), msat_make_number(menv, "10"))), enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[4]), msat_make_number(menv, "-10"))))
