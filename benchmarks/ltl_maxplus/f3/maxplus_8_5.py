...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_7 - x_4 >= 2) & (G (x_3 - x_2 > 6))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[4]), msat_make_number(menv, "2")), enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "6")))))
