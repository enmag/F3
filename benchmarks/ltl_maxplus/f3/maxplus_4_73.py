...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (X (x_3 - x_2 > -18))) U (x_2 - x_3 >= 13))
    ltl = enc.make_U(enc.make_X(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "-18")))), msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[3]), msat_make_number(menv, "13")))
