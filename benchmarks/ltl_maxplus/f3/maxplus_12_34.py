...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (X (x_1 - x_11 >= -2))) U (! (x_8 - x_9 >= 2)))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[11]), msat_make_number(menv, "-2")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[8], xs[9]), msat_make_number(menv, "2"))))))
