...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (G (x_3 - x_1 >= 9))) U (! (X (x_1 - x_2 >= -18)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "9")))), msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[2]), msat_make_number(menv, "-18"))))))
