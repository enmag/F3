...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (X (! ((! (x_11 - x_4 > -11)) U (! (x_2 - x_7 >= 15))))))
    ltl = enc.make_X(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[4]), msat_make_number(menv, "-11"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[7]), msat_make_number(menv, "15")))))))
