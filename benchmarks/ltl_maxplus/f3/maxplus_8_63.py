...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_0 - x_6 > -19) | (! ((! (x_4 - x_0 > 13)) U (! (x_0 - x_4 >= -14)))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[6]), msat_make_number(menv, "-19")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[0]), msat_make_number(menv, "13"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[4]), msat_make_number(menv, "-14"))))))
