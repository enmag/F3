...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_4 - x_10 >= -17)) U (! (x_1 - x_11 > -2))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[10]), msat_make_number(menv, "-17"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[11]), msat_make_number(menv, "-2")))))))
