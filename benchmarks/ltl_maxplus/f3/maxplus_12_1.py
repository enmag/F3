...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_3 - x_11 >= 12) | (X (G (x_5 - x_4 >= 11))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[11]), msat_make_number(menv, "12")), enc.make_X(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[4]), msat_make_number(menv, "11")))))
