...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: ((x_2 - x_1 >= -13) | (! ((! (x_1 - x_2 > 19)) U (! (x_1 - x_3 > 4)))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "-13")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[2]), msat_make_number(menv, "19"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[3]), msat_make_number(menv, "4"))))))
