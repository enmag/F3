...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (X (x_18 - x_4 >= 19))) U (! (F (x_10 - x_3 >= 2)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[18], xs[4]), msat_make_number(menv, "19")))), msat_make_not(menv, enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[10], xs[3]), msat_make_number(menv, "2"))))))
