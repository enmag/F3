...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_3 - x_2 > -12)) U (! (G (F (x_4 - x_2 >= 8))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "-12"))), msat_make_not(menv, enc.make_G(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[2]), msat_make_number(menv, "8")))))))
