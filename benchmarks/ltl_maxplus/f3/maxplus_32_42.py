...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! ((x_7 - x_0 > -6) & (x_5 - x_9 >= -7))) U (! (x_16 - x_6 > -9))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[0]), msat_make_number(menv, "-6")), msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[9]), msat_make_number(menv, "-7")))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[16], xs[6]), msat_make_number(menv, "-9")))))
