...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr8 = msat_make_plus(menv, xs[25], n_19_0)
    ...
    # ltl property: (! ((! (G (x_1 - x_16 >= -17))) U (! (X (x_17 - x_0 >= -3)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[16]), msat_make_number(menv, "-17")))), msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[17], xs[0]), msat_make_number(menv, "-3"))))))
