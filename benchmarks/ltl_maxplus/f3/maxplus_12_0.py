...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((F (x_3 - x_7 >= -15)) U (x_1 - x_4 > 16)))
    ltl = enc.make_X(enc.make_U(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[7]), msat_make_number(menv, "-15"))), msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[4]), msat_make_number(menv, "16"))))
