...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (G ((x_21 - x_19 > 0) & (x_6 - x_2 > -8))))
    ltl = enc.make_F(enc.make_G(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[21], xs[19]), msat_make_number(menv, "0")), msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[2]), msat_make_number(menv, "-8")))))
