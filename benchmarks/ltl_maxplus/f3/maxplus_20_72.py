...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (x_11 - x_1 > 9)) U (! (G (x_18 - x_19 > 7))))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[1]), msat_make_number(menv, "9"))), msat_make_not(menv, enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[18], xs[19]), msat_make_number(menv, "7")))))))
