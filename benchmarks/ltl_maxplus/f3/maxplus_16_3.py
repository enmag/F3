...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (X ((x_13 - x_1 >= 2) & (x_15 - x_3 > 6))))
    ltl = enc.make_F(enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[13], xs[1]), msat_make_number(menv, "2")), msat_make_gt(menv, msat_make_minus(menv, xs[15], xs[3]), msat_make_number(menv, "6")))))
