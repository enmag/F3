...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (X (F (x_2 - x_3 >= 20)))) U (! (x_3 - x_4 >= -5))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[3]), msat_make_number(menv, "20"))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[4]), msat_make_number(menv, "-5")))))
