...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_1 - x_8 >= -11) | (! ((! (x_5 - x_3 >= -7)) U (! (x_8 - x_7 >= 3)))))
    ltl = msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[8]), msat_make_number(menv, "-11")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[3]), msat_make_number(menv, "-7"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[8], xs[7]), msat_make_number(menv, "3"))))))
