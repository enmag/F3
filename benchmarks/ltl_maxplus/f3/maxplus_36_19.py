...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_26 - x_5 > 12)) U (! (x_20 - x_9 >= -17))))) U (! (x_12 - x_7 >= 9))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[26], xs[5]), msat_make_number(menv, "12"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[20], xs[9]), msat_make_number(menv, "-17")))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[7]), msat_make_number(menv, "9")))))
