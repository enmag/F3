...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G ((x_5 - x_7 >= 15) | (X (x_3 - x_5 > -5))))
    ltl = enc.make_G(msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[7]), msat_make_number(menv, "15")), enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[5]), msat_make_number(menv, "-5")))))
