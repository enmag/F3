...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_3 - x_1 > -9) & (G (x_6 - x_4 > 0))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "-9")), enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[4]), msat_make_number(menv, "0")))))
