...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_6 - x_9 > -3)) U (! (x_19 - x_16 >= -18))))) U (! (x_27 - x_9 > 5))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[6], xs[9]), msat_make_number(menv, "-3"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[19], xs[16]), msat_make_number(menv, "-18")))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[27], xs[9]), msat_make_number(menv, "5")))))
