...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((F (x_16 - x_11 >= 0)) U (x_1 - x_0 >= 20)))
    ltl = enc.make_X(enc.make_U(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[16], xs[11]), msat_make_number(menv, "0"))), msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[0]), msat_make_number(menv, "20"))))
