...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (X (x_12 - x_6 > 10))) U (! (x_4 - x_22 >= 18)))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[12], xs[6]), msat_make_number(menv, "10")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[22]), msat_make_number(menv, "18"))))))
