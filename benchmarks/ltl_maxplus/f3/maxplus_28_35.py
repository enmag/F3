...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_17 - x_20 >= -11)) & (F (x_21 - x_1 > 19)))
    ltl = msat_make_and(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[17], xs[20]), msat_make_number(menv, "-11"))), enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[21], xs[1]), msat_make_number(menv, "19"))))
