...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (x_1 - x_2 >= -13)) U (! (G (x_3 - x_2 > 11))))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[2]), msat_make_number(menv, "-13"))), msat_make_not(menv, enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "11")))))))
