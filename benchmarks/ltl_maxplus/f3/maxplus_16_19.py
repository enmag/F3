...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_4 - x_11 > 14)) U (! (x_1 - x_0 > 14))))) U (! (x_1 - x_8 >= -11))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[11]), msat_make_number(menv, "14"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[0]), msat_make_number(menv, "14")))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[8]), msat_make_number(menv, "-11")))))
