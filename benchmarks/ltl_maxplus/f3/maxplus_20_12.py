...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_19 - x_11 > -15)) U (! (x_1 - x_13 > 11))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[19], xs[11]), msat_make_number(menv, "-15"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[13]), msat_make_number(menv, "11")))))))
