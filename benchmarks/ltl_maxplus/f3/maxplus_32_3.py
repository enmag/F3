...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (X ((x_31 - x_22 >= -11) & (x_14 - x_22 >= 8))))
    ltl = enc.make_F(enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[31], xs[22]), msat_make_number(menv, "-11")), msat_make_geq(menv, msat_make_minus(menv, xs[14], xs[22]), msat_make_number(menv, "8")))))
