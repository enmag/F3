...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (X (x_7 - x_3 > -8))) U (! (x_6 - x_9 >= 19)))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[3]), msat_make_number(menv, "-8")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[9]), msat_make_number(menv, "19"))))))
