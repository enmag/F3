...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_0 - x_3 >= 18) & (G (x_0 - x_2 >= 3))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "18")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[2]), msat_make_number(menv, "3")))))
