...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (X (x_8 - x_17 > 16))) U (! (x_16 - x_22 > -11)))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[17]), msat_make_number(menv, "16")))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[16], xs[22]), msat_make_number(menv, "-11"))))))
