...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (X ((x_11 - x_13 >= 5) & (x_25 - x_2 > 14))))
    ltl = enc.make_F(enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[11], xs[13]), msat_make_number(menv, "5")), msat_make_gt(menv, msat_make_minus(menv, xs[25], xs[2]), msat_make_number(menv, "14")))))
