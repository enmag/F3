...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr11 = msat_make_plus(menv, xs[15], n_6_0)
    ...
    # ltl property: (! ((! (X (x_27 - x_1 > 16))) U (! (F (x_26 - x_1 >= -18)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[27], xs[1]), msat_make_number(menv, "16")))), msat_make_not(menv, enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[26], xs[1]), msat_make_number(menv, "-18"))))))
