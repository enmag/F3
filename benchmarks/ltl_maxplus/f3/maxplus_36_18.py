...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_28 - x_16 >= 17) | (X (x_30 - x_7 >= -19))))
    ltl = enc.make_X(msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[28], xs[16]), msat_make_number(menv, "17")), enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[30], xs[7]), msat_make_number(menv, "-19")))))
