...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_0 - x_1 >= -4)) U (! ((x_3 - x_2 >= 14) | (x_2 - x_1 >= -7)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[1]), msat_make_number(menv, "-4"))), msat_make_not(menv, msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[2]), msat_make_number(menv, "14")), msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[1]), msat_make_number(menv, "-7"))))))
