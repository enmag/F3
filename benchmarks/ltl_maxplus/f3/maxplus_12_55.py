...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_7 - x_11 > -19)) U (! (X (X (x_10 - x_1 >= 2))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[11]), msat_make_number(menv, "-19"))), msat_make_not(menv, enc.make_X(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[10], xs[1]), msat_make_number(menv, "2")))))))
