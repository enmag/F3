...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (F (x_4 - x_11 > 0))) U (! (x_9 - x_10 >= 11)))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[11]), msat_make_number(menv, "0")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[10]), msat_make_number(menv, "11"))))))
