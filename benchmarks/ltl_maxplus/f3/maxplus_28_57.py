...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (X (x_4 - x_1 >= -3))) U (! (x_22 - x_14 > 20)))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[1]), msat_make_number(menv, "-3")))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[22], xs[14]), msat_make_number(menv, "20"))))))
