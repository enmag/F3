...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_7 - x_2 > -1) | ((x_4 - x_1 >= 14) & (x_1 - x_6 >= -12)))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[2]), msat_make_number(menv, "-1")), msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[1]), msat_make_number(menv, "14")), msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[6]), msat_make_number(menv, "-12"))))
