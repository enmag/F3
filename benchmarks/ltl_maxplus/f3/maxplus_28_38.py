...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (X (! ((! (x_22 - x_13 > -11)) U (! (x_23 - x_18 > 20))))))
    ltl = enc.make_G(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[22], xs[13]), msat_make_number(menv, "-11"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[23], xs[18]), msat_make_number(menv, "20")))))))
