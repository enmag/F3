...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_11 - x_2 >= 18)) U (! ((x_0 - x_2 > 3) | (x_0 - x_9 >= -4)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[11], xs[2]), msat_make_number(menv, "18"))), msat_make_not(menv, msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[2]), msat_make_number(menv, "3")), msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[9]), msat_make_number(menv, "-4"))))))
