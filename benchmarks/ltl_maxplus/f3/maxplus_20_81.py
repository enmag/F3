...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_11 - x_16 > -17)) U (! (x_15 - x_4 > 7))))) U (! (x_11 - x_16 > -17))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[16]), msat_make_number(menv, "-17"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[15], xs[4]), msat_make_number(menv, "7")))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[11], xs[16]), msat_make_number(menv, "-17")))))
