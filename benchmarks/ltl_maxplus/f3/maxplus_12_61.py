...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (X (x_1 - x_3 > 16))) U (x_10 - x_7 >= -4))
    ltl = enc.make_U(enc.make_X(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[3]), msat_make_number(menv, "16")))), msat_make_geq(menv, msat_make_minus(menv, xs[10], xs[7]), msat_make_number(menv, "-4")))
