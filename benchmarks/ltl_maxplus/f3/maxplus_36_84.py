...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_8 - x_15 > -9) | (! ((! (x_34 - x_19 > 20)) U (! (x_13 - x_19 >= -3)))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[8], xs[15]), msat_make_number(menv, "-9")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[34], xs[19]), msat_make_number(menv, "20"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[13], xs[19]), msat_make_number(menv, "-3"))))))
