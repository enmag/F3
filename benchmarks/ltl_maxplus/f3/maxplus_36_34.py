...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (X (x_34 - x_33 >= 20))) U (! (x_2 - x_17 > -19)))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[34], xs[33]), msat_make_number(menv, "20")))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[17]), msat_make_number(menv, "-19"))))))
