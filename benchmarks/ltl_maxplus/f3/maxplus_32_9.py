...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (F (X (x_2 - x_19 >= 11)))) U (! (x_9 - x_25 >= -1))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[19]), msat_make_number(menv, "11"))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[25]), msat_make_number(menv, "-1")))))
