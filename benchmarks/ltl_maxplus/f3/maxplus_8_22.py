...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G ((G (x_5 - x_7 >= -6)) U (x_7 - x_0 >= 16)))
    ltl = enc.make_G(enc.make_U(enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[5], xs[7]), msat_make_number(menv, "-6"))), msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[0]), msat_make_number(menv, "16"))))
