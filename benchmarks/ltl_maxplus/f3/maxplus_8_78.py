...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (F (x_3 - x_4 > 4))) U (! (x_1 - x_6 >= -11)))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[4]), msat_make_number(menv, "4")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[6]), msat_make_number(menv, "-11"))))))
