...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (X (X (! ((! (x_5 - x_2 > 14)) U (! (x_2 - x_7 >= 20))))))
    ltl = enc.make_X(enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[5], xs[2]), msat_make_number(menv, "14"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[7]), msat_make_number(menv, "20")))))))
