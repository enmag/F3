...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (X (x_10 - x_11 >= 3))) U (! (x_2 - x_5 >= -14)))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[10], xs[11]), msat_make_number(menv, "3")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[5]), msat_make_number(menv, "-14"))))))
