...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (x_15 - x_9 >= -2)) | (F (x_12 - x_11 >= 16)))
    ltl = msat_make_or(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[15], xs[9]), msat_make_number(menv, "-2"))), enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[11]), msat_make_number(menv, "16"))))
