...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (G (x_7 - x_1 > 20))) U (! (X (x_7 - x_6 > 2)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[1]), msat_make_number(menv, "20")))), msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[6]), msat_make_number(menv, "2"))))))
