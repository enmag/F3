...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: ((x_3 - x_1 > 16) | (! ((! (x_2 - x_3 > 17)) U (! (x_3 - x_1 >= 3)))))
    ltl = msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "16")), msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[2], xs[3]), msat_make_number(menv, "17"))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "3"))))))
