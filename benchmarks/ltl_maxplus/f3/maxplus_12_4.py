...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (x_10 - x_0 > 12)) | (G (x_4 - x_0 >= 9)))
    ltl = msat_make_or(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[10], xs[0]), msat_make_number(menv, "12"))), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[0]), msat_make_number(menv, "9"))))
