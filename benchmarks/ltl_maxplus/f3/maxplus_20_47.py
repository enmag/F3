...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (G ((x_4 - x_8 > -9) & (x_12 - x_4 > 0))))
    ltl = enc.make_F(enc.make_G(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[8]), msat_make_number(menv, "-9")), msat_make_gt(menv, msat_make_minus(menv, xs[12], xs[4]), msat_make_number(menv, "0")))))
