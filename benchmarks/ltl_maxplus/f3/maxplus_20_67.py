...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_17 - x_8 >= -10) & (X (x_17 - x_14 >= 10))))
    ltl = enc.make_X(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[17], xs[8]), msat_make_number(menv, "-10")), enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[17], xs[14]), msat_make_number(menv, "10")))))
