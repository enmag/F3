...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (X ((x_0 - x_1 > -14) & (x_0 - x_1 >= -7))))
    ltl = enc.make_F(enc.make_X(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[1]), msat_make_number(menv, "-14")), msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[1]), msat_make_number(menv, "-7")))))
