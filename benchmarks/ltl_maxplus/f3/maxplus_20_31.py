...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_7 - x_14 > -9)) U (! ((x_18 - x_5 > 12) | (x_9 - x_16 > 12)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[7], xs[14]), msat_make_number(menv, "-9"))), msat_make_not(menv, msat_make_or(menv, msat_make_gt(menv, msat_make_minus(menv, xs[18], xs[5]), msat_make_number(menv, "12")), msat_make_gt(menv, msat_make_minus(menv, xs[9], xs[16]), msat_make_number(menv, "12"))))))
