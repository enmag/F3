...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    expr8 = msat_make_plus(menv, xs[8], n_4_0)
    expr9 = msat_make_plus(menv, xs[9], n_3_0)
    expr10 = msat_make_plus(menv, xs[12], n_20_0)
    ...
    # ltl property: (! ((! (X (F (x_19 - x_0 >= -13)))) U (! (x_25 - x_10 >= 9))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[19], xs[0]), msat_make_number(menv, "-13"))))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[25], xs[10]), msat_make_number(menv, "9")))))
