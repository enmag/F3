...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (x_3 - x_1 >= 0)) U (! (X (x_7 - x_23 >= -5))))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "0"))), msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[23]), msat_make_number(menv, "-5")))))))
