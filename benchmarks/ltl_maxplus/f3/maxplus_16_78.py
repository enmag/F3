...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X (! ((! (F (x_15 - x_4 > -10))) U (! (x_9 - x_4 >= -8)))))
    ltl = enc.make_X(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[15], xs[4]), msat_make_number(menv, "-10")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[4]), msat_make_number(menv, "-8"))))))
