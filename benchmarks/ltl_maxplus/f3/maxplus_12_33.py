...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((X (x_4 - x_9 > 7)) U (X (x_9 - x_11 >= -6)))
    ltl = enc.make_U(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[4], xs[9]), msat_make_number(menv, "7"))), enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[9], xs[11]), msat_make_number(menv, "-6"))))
