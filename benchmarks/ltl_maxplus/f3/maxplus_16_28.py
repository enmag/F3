...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (X ((x_13 - x_3 > -8) & (G (x_14 - x_9 >= -9))))
    ltl = enc.make_X(msat_make_and(menv, msat_make_gt(menv, msat_make_minus(menv, xs[13], xs[3]), msat_make_number(menv, "-8")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[14], xs[9]), msat_make_number(menv, "-9")))))
