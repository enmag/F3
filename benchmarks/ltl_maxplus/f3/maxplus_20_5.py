...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F ((x_4 - x_19 >= 13) & (G (x_15 - x_0 >= -8))))
    ltl = enc.make_F(msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[19]), msat_make_number(menv, "13")), enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[15], xs[0]), msat_make_number(menv, "-8")))))
