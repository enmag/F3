...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (G (F (X (x_0 - x_2 > 5)))))
    ltl = enc.make_F(enc.make_G(enc.make_F(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[2]), msat_make_number(menv, "5"))))))
