...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (! ((! (x_7 - x_3 >= 12)) U (! (x_21 - x_5 > -1))))) U (! (x_18 - x_24 > 3))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[7], xs[3]), msat_make_number(menv, "12"))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[21], xs[5]), msat_make_number(menv, "-1")))))), msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[18], xs[24]), msat_make_number(menv, "3")))))
