...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! ((x_0 - x_6 >= 3) & (x_0 - x_11 > 2))) U (! (x_4 - x_1 >= 2))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[0], xs[6]), msat_make_number(menv, "3")), msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[11]), msat_make_number(menv, "2")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[1]), msat_make_number(menv, "2")))))
