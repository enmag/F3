...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((x_6 - x_5 >= 11) & (G (F (x_2 - x_7 >= -5))))
    ltl = msat_make_and(menv, msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[5]), msat_make_number(menv, "11")), enc.make_G(enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[7]), msat_make_number(menv, "-5")))))
