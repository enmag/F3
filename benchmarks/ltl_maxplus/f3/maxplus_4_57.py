...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (X (x_1 - x_0 >= 12))) U (! (x_2 - x_0 >= 13)))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[0]), msat_make_number(menv, "12")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[2], xs[0]), msat_make_number(menv, "13"))))))
