...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (! ((! (G (x_6 - x_8 >= 13))) U (! (X (x_1 - x_8 >= 6)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[8]), msat_make_number(menv, "13")))), msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[8]), msat_make_number(menv, "6"))))))
