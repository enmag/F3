...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (X (x_3 - x_0 >= -20))) U (! (F (x_3 - x_1 >= 3)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-20")))), msat_make_not(menv, enc.make_F(msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[1]), msat_make_number(menv, "3"))))))
