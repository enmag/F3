...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (X (x_12 - x_17 >= 3))) U (! (x_18 - x_8 >= 0)))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_geq(menv, msat_make_minus(menv, xs[12], xs[17]), msat_make_number(menv, "3")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[18], xs[8]), msat_make_number(menv, "0"))))))
