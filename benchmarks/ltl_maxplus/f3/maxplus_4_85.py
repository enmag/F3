...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: ((G (x_1 - x_2 >= -20)) | (F (x_0 - x_3 > 12)))
    ltl = msat_make_or(menv, enc.make_G(msat_make_geq(menv, msat_make_minus(menv, xs[1], xs[2]), msat_make_number(menv, "-20"))), enc.make_F(msat_make_gt(menv, msat_make_minus(menv, xs[0], xs[3]), msat_make_number(menv, "12"))))
