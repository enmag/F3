...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (F (! ((! (X (x_9 - x_1 > 7))) U (! (x_4 - x_15 >= 10)))))
    ltl = enc.make_F(msat_make_not(menv, enc.make_U(msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[9], xs[1]), msat_make_number(menv, "7")))), msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[4], xs[15]), msat_make_number(menv, "10"))))))
