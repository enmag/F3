...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G ((X (x_1 - x_2 > -10)) U (x_3 - x_0 > -4)))
    ltl = enc.make_G(enc.make_U(enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[2]), msat_make_number(menv, "-10"))), msat_make_gt(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-4"))))
