...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_8 - x_11 >= -11)) U (! (X (G (x_1 - x_11 > -11))))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[8], xs[11]), msat_make_number(menv, "-11"))), msat_make_not(menv, enc.make_X(enc.make_G(msat_make_gt(menv, msat_make_minus(menv, xs[1], xs[11]), msat_make_number(menv, "-11")))))))
