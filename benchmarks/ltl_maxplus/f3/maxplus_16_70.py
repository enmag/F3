...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> Tuple[Iterable, msat_term,
    ...
    # ltl property: (G (! ((! (x_14 - x_8 > -17)) U (! (X (x_15 - x_3 > -9))))))
    ltl = enc.make_G(msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_gt(menv, msat_make_minus(menv, xs[14], xs[8]), msat_make_number(menv, "-17"))), msat_make_not(menv, enc.make_X(msat_make_gt(menv, msat_make_minus(menv, xs[15], xs[3]), msat_make_number(menv, "-9")))))))
