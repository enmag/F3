...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    # ltl property: (! ((! (x_6 - x_7 >= -5)) U (! ((x_6 - x_3 >= -20) | (x_3 - x_0 >= -14)))))
    ltl = msat_make_not(menv, enc.make_U(msat_make_not(menv, msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[7]), msat_make_number(menv, "-5"))), msat_make_not(menv, msat_make_or(menv, msat_make_geq(menv, msat_make_minus(menv, xs[6], xs[3]), msat_make_number(menv, "-20")), msat_make_geq(menv, msat_make_minus(menv, xs[3], xs[0]), msat_make_number(menv, "-14"))))))
