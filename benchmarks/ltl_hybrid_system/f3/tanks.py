...
def check_ltl(menv: msat_env, enc: LTLEncoder):
    ...
    pipes = [
        Pipe(menv, "pipe{}".format(i), delta, x_delta, max_flows[i],
    ...
    tanks = [
        Tank(menv, "tank{}".format(i), delta, x_delta, max_vols[i], flows[i],
    ...
    # transition relation.
    trans = msat_make_geq(menv, x_delta, _0)
    for comp in components:
...
class Tank:
    ...
    def __init__(self, menv: msat_env, name: str, delta,
        ...
        self.vol, self.x_vol = decl_consts(menv,"{}_vol".format(name), real_type)
...
class Pipe:
    ...
    def __init__(self, menv: msat_env, name: str, delta,
        ...
        self.mode, self.x_mode = decl_consts(menv,"{}_mode".format(name),
                                             int_type)
        self.flow, self.x_flow = decl_consts(menv, "{}_flow".format(name),
                                             real_type)
        self.d_flow, self.x_d_flow = decl_consts(menv, "{}_dflow".format(name),
                                                 real_type)
    ...
    def invar(self):
        ...
        _0 = msat_make_number(menv, "0")
        _1 = msat_make_number(menv, "1")
        lhs = msat_make_or(menv, self.close, self.open)
        rhs = msat_make_equal(menv, self.d_flow, _0)
        res = msat_make_impl(menv, lhs, rhs)

        cond = lhs
        lhs = msat_make_and(menv, cond, self.opening)
        rhs = msat_make_equal(menv, self.d_flow, _1)
        res = msat_make_and(menv, res, msat_make_impl(menv, lhs, rhs))

        lhs = msat_make_and(menv, cond, msat_make_not(menv, self.opening))
        rhs = msat_make_equal(menv, self.d_flow, m_1)
        res = msat_make_and(menv, res, msat_make_impl(menv, lhs, rhs))

        lhs = self.close
        rhs = msat_make_equal(menv, self.flow, _0)
        res = msat_make_and(menv, res, msat_make_impl(menv, lhs, rhs))

        lhs = self.open
        rhs = msat_make_equal(menv, self.flow, self.max_flow)
        res = msat_make_and(menv, res, msat_make_impl(menv, lhs, rhs))
    ...
    def x_invar(self):
        ...
        _0 = msat_make_number(menv, "0")
        _1 = msat_make_number(menv, "1")
        lhs = msat_make_or(menv, self.x_close, self.x_open)
        rhs = msat_make_equal(menv, self.x_d_flow, _0)
        res = msat_make_impl(menv, lhs, rhs)

        cond = lhs
        lhs = msat_make_and(menv, cond, self.x_opening)
        rhs = msat_make_equal(menv, self.x_d_flow, _1)
        res = msat_make_and(menv, res, msat_make_impl(menv, lhs, rhs))

        lhs = msat_make_and(menv, cond, msat_make_not(menv, self.x_opening))
        rhs = msat_make_equal(menv, self.x_d_flow, m_1)
        res = msat_make_and(menv, res, msat_make_impl(menv, lhs, rhs))

        lhs = self.x_close
        rhs = msat_make_equal(menv, self.x_flow, _0)
        res = msat_make_and(menv, res, msat_make_impl(menv, lhs, rhs))

        lhs = self.x_open
        rhs = msat_make_equal(menv, self.x_flow, self.max_flow)
        res = msat_make_and(menv, res, msat_make_impl(menv, lhs, rhs))
