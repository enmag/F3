...
def check_ltl(menv: msat_env, enc: LTLEncoder):
    ...
    avg_sm = sms[0].sm
    for sm in sms[1:]:
        avg_sm = msat_make_plus(menv, avg_sm, sm.sm)
    avg_sm = msat_make_divide(menv, avg_sm,
                              msat_make_number(menv, str(len(sms))))
    fairness = msat_make_equal(menv, cms[0].cm, avg_sm)
    for cm in cms[1:]:
        fairness = msat_make_and(menv, fairness,
                                 msat_make_equal(menv, cm.cm, avg_sm))
