...
def check_ltl(menv: msat_env, enc: LTLEncoder):
    ...
    delay = msat_make_number(menv, str(DELAY))
    drifts = [_3, _2, _4, _1, _10]

    delta, x_delta = decl_consts(menv, delta_name, real_type)
    cms = [
        ComprMaster(menv, f"cm{i}", delay, delta, x_delta)
        for i in range(NUM_COMPR_MASTER)
    ]
    sms = [
        SyncMaster(menv, f"sm{i}", drifts[i], delta, x_delta)
...
class ComprMaster:
    ...
    def pysmt_symbs(env: PysmtEnv, name: str):
        ...
        return [mgr.Symbol(f"{name}{ComprMaster._MODE_NAME}", types.INT),
    ...
    def __init__(self, menv: msat_env, name: str, delay,
        ...
        self.mode, self.x_mode = decl_consts(menv,
                                             f"{name}{ComprMaster._MODE_NAME}",
                                             int_type)
        self.cm, self.x_cm = decl_consts(menv, f"{name}{ComprMaster._CM_NAME}",
                                         real_type)
        self.x, self.x_x = decl_consts(menv, f"{name}{ComprMaster._X_NAME}",
                                       real_type)
    ...
    def waiting(self):
        ...
        return msat_make_equal(menv, self.mode,
                               msat_make_number(menv, str(ComprMaster._WAITING)))
    ...
    def receive(self):
        ...
        menv = self.menv
        return msat_make_equal(menv, self.mode,
                               msat_make_number(menv, str(ComprMaster._RECEIVE)))
    ...
    def correct1(self):
        ...
        menv = self.menv
        return msat_make_equal(menv, self.mode,
                               msat_make_number(menv, str(ComprMaster._CORRECT1)))
    ...
    def correct2(self):
        ...
        menv = self.menv
        return msat_make_equal(menv, self.mode,
                               msat_make_number(menv, str(ComprMaster._CORRECT2)))
...
class SyncMaster:
    ...
    def hints(env: PysmtEnv, name: str, delta, symbs):
        ...
        loc2.set_progress(3, eq_0=x_sync2)
        ...
        loc3.set_progress(0, eq_0=x_work)
    ...
    def __init__(self, menv: msat_env, name: str, drift,
        ...
        real_type = msat_get_rational_type(menv)
        self.mode, self.x_mode = decl_consts(menv,
                                             f"{name}{SyncMaster._MODE_NAME}",
                                             int_type)
        self.sm, self.x_sm = decl_consts(menv,
                                         f"{name}{SyncMaster._SM_NAME}",
                                         real_type)
    ...
    def work(self):
        ...
        menv = self.menv
        return msat_make_equal(menv, self.mode,
                               msat_make_number(menv, str(SyncMaster._WORK)))
    ...
    def send(self):
        ...
        menv = self.menv
        return msat_make_equal(menv, self.mode,
                               msat_make_number(menv, str(SyncMaster._SEND)))
    ...
    def sync1(self):
        ...
        menv = self.menv
        return msat_make_equal(menv, self.mode,
                               msat_make_number(menv, str(SyncMaster._SYNC1)))
    ...
    def sync2(self):
        ...
        menv = self.menv
        return msat_make_equal(menv, self.mode,
                               msat_make_number(menv, str(SyncMaster._SYNC2)))
...
def hints(env: PysmtEnv):
    ...
    delay = mgr.Real(DELAY)
    symbs = set([delta])
    for i in range(NUM_COMPR_MASTER):
        symbs.update(ComprMaster.pysmt_symbs(env, f"cm{i}"))
    for i in range(NUM_SYNC_MASTER):
        symbs.update(SyncMaster.pysmt_symbs(env, f"sm{i}"))
    symbs = frozenset(symbs)

    r_0 = mgr.Real(0)

    x_delta = symb_to_next(mgr, delta)
    loc0 = Location(env, mgr.Equals(delta, delay))
    loc0.set_progress(1, eq_0=mgr.Equals(x_delta, r_0))
    loc1 = Location(env, mgr.Equals(delta, r_0),
                    stutterT=mgr.Equals(x_delta, delta))
    loc1.set_progress(0, eq_0=mgr.Equals(x_delta, delay))
