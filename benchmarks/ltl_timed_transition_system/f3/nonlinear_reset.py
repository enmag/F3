...
def check_ltl(menv: msat_env, enc: LTLEncoder) -> (Iterable, msat_term,
    ...
    bound, x_bound = decl_consts(menv, "bound", real_type)
    counter, x_counter = decl_consts(menv, "counter", int_type)
    delta, x_delta = decl_consts(menv, delta_name, real_type)

    curr2next = {c: x_c, bound: x_bound, delta: x_delta, counter: x_counter}
    ...
    init = msat_make_and(menv, init,
                         msat_make_and(menv,
                                       ...
                                       msat_make_equal(menv, counter, zero)))

    # delta > 0 -> c' = c + delta & counter' = counter
    ...
    # c = bound -> (c' = pow(counter, 2) & counter' = counter + 1)
    lhs = msat_make_and(menv, disc_t, msat_make_equal(menv, c, bound))
    rhs = msat_make_and(
        menv,
        msat_make_equal(menv, x_counter, msat_make_plus(menv, counter, one)),
        msat_make_equal(menv, x_c,
                        msat_make_times(menv, counter, counter)))
    trans = msat_make_and(menv, trans,
                          msat_make_impl(menv, lhs, rhs))
    # c != bound -> (c' = c & counter' = counter)
    lhs = msat_make_and(menv, disc_t,
                        msat_make_not(menv, msat_make_equal(menv, c, bound)))
    rhs = msat_make_and(menv, msat_make_equal(menv, x_c, c),
                        msat_make_equal(menv, x_counter, counter))
    trans = msat_make_and(menv, trans,
                          msat_make_impl(menv, lhs, rhs))

    # F G (c != bound | delta > 0)
    ltl = msat_make_not(menv, msat_make_equal(menv, c, bound))
    ltl = msat_make_or(menv, ltl,
                       msat_make_gt(menv, delta, zero))
    ltl = enc.make_F(enc.make_G(ltl))
