...
def transition_system(env: PysmtEnv) -> Tuple[FrozenSet[FNode], FNode, FNode, FNode]:
    ...
    cfg = mgr.And(
        ...
        # pc = 0 & !(x <= y - 1) : -1,
        mgr.Implies(mgr.And(pcs[0], mgr.Not(mgr.LE(x, mgr.Minus(y, ints[1])))),
                    x_pcend),
        # pc = 0 & x <= y - 1 : 1,
        mgr.Implies(mgr.And(pcs[0], mgr.LE(x, mgr.Minus(y, ints[1]))),
                    x_pcs[1]),
        # pc = 1 & !(x <= y - 1) : -1,
