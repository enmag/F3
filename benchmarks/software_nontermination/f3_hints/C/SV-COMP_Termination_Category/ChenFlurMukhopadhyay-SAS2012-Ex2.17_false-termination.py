...
def hints(env: PysmtEnv) -> FrozenSet[Hint]:
    ...
    l0 = Location(env, mgr.Equals(x, m_1), mgr.GE(y, i_0), stutterT=stutter)
    l0.set_progress(0, eq_0=mgr.Equals(x_x, mgr.Times(m_1, y)))
    h_x = Hint("h_x", env, frozenset([x]), symbs)
    h_x.set_locs([l0])

    x_y = symb_to_next(mgr, y)
    stutter = mgr.Equals(x_y, y)
    l0 = Location(env, mgr.GE(y, i_0), stutterT=stutter)
    l0.set_progress(0, eq_0=mgr.Equals(x_y, mgr.Plus(y, i_1)))
