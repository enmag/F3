...
def transition_system(env: PysmtEnv) -> Tuple[FrozenSet[FNode], FNode, FNode,
    ...
    trans.append(mgr.Implies(pcs[2],
                             mgr.And(same_x,
                                     mgr.Equals(x_y, mgr.Times(m_2, y)))))
    # pc = end -> same
...
def hints(env: PysmtEnv) -> FrozenSet[Hint]:
    ...
    loc = Location(env, mgr.Equals(x, m_2), mgr.Equals(y, i_0), stutterT=stutter)
    ...
    loc = Location(env, mgr.Equals(y, i_0), stutterT=stutter)
    loc.set_progress(0, eq_0=mgr.Equals(x_y, mgr.Times(m_2, y)))
