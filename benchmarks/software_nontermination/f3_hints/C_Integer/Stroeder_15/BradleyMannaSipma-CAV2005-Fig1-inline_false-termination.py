...
def transition_system(env: PysmtEnv) -> Tuple[FrozenSet[FNode], FNode, FNode,
    ...
    x_y1 = symb_to_next(mgr, y1)
    x_y2 = symb_to_next(mgr, y2)
    ...
    trans.append(mgr.Implies(pcs[3],
                             mgr.And(mgr.Equals(x_y1, mgr.Minus(y1, y2)),
                                     same_y2)))
    # pc = 4 -> same_y1 & y2' = y2 - y1
    trans.append(mgr.Implies(pcs[4],
                             mgr.And(same_y1,
                                     mgr.Equals(x_y2, mgr.Minus(y2, y1)))))
    # pc = end -> same
...
def hints(env: PysmtEnv) -> FrozenSet[Hint]:
    ...
    y1 = mgr.Symbol("y1", types.INT)
    y2 = mgr.Symbol("y2", types.INT)
    symbs = frozenset([pc, y1, y2])

    i_0 = mgr.Int(0)
    i_1 = mgr.Int(1)

    x_y1 = symb_to_next(mgr, y1)
    stutter = mgr.Equals(x_y1, y1)
    loc = Location(env, mgr.Equals(y1, i_1), mgr.Equals(y2, i_0),
                   stutterT=stutter)
    loc.set_progress(0, eq_0=mgr.Equals(x_y1, mgr.Minus(y1, y2)))
    h_y1 = Hint("h_y1", env, frozenset([y1]), symbs)
    h_y1.set_locs([loc])

    x_y2 = symb_to_next(mgr, y2)
    stutter = mgr.Equals(x_y2, y2)
    loc = Location(env, mgr.Equals(y2, i_0))
    loc.set_progress(0, eq_0=mgr.Equals(x_y2, y2))
    h_y2 = Hint("h_y2", env, frozenset([y2]), symbs)
    h_y2.set_locs([loc])

    return frozenset([h_y1, h_y2])
