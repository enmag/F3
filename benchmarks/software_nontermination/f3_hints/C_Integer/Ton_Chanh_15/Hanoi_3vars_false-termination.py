...
def hints(env: PysmtEnv) -> FrozenSet[Hint]:
    ...
    loc.set_progress(0, eq_0=mgr.Equals(x_y, mgr.Plus(y, z)))
    h_y = Hint("h_y", env, frozenset([y]), symbs)
    h_y.set_locs([loc])

    x_z = symb_to_next(mgr, z)
    stutter = mgr.Equals(x_z, z)
    loc = Location(env, mgr.GE(z, i_0), stutterT=stutter)
    loc.set_progress(0, eq_0=mgr.Equals(x_z, mgr.Plus(z, i_1)))
