...
def encode_diverging(menv, enc, div_s, symbs) -> \
        Tuple[msat_term, msat_term, msat_term, msat_term]:
    assert symbs is not None
    monitor_name = f"_div_{msat_term_repr(div_s)}"
    real_type = msat_get_rational_type(menv)
    monitor = msat_make_constant(menv,
                                 msat_declare_function(menv, monitor_name,
                                                       real_type))
    x_monitor = msat_make_constant(menv,
                                   msat_declare_function(menv,
                                                         name2next(monitor_name),
                                                         real_type))
    symbs[monitor] = x_monitor
    one = msat_make_number(menv, "1")
    if get_div_mode() is True:
        # monitor = div_s
        init = msat_make_equal(menv, monitor, div_s)
            ...
            msat_make_impl(menv, msat_make_not(menv, monitor_geq_1),
                           msat_make_equal(menv, x_monitor,
                                           msat_make_plus(menv, monitor,
                                                          div_s))))
        # G F monitor >= 1
        ltl = enc.make_G(enc.make_F(monitor_geq_1))
    else:
        zero = msat_make_number(menv, "0")
        # monitor = 0
        init = msat_make_equal(menv, monitor, zero)
        ...
        ltl = enc.make_G(enc.make_F(msat_make_and(menv, init,
                                                  msat_make_lt(menv, zero,
                                                               div_s))))

    return monitor, init, trans, ltl
