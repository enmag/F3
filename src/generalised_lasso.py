...
def is_generalised_lasso(env: PysmtEnv, loop: list, symbols: frozenset,
                         trans: FNode,
    ...
    assert all(isinstance(k, FNode) for state in loop for k in state.keys())
    assert all(k in symbols for state in loop for k in state.keys())
    ...
    preds = frozenset(filter(lambda e: not e.is_true() and not e.is_false(),
                             (simpl(subst(p, assign)) for assign in conc_assigns
                              for s in symbols for p in trans.get_atoms())))
    assert all(p.get_free_variables() <= (symbols - conc_lasso_symbs) |
