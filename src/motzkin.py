...
def _matrix_product(env: PysmtEnv, keys: list, coeffs: list,
                    ineqs: list, td: TimesDistributor,
                    expr_type=types.REAL) -> Tuple[List[Expr], Expr]:
    """Return FNodes representing product of coefs * ineqs

    Ineqs is: Ax < b or Ax <= b, returns coefs * A, coefs * b.
    Missing keys in ineq are implicitely 0, hence do no generate
    constraints.
    """
...
def motzkin_transpose(env: PysmtEnv, ineqs: Iterable, td: TimesDistributor,
                      cn: Canonizer, params=None) -> Generator[FNode,
                                                               None, None]:
    ...
    for curr in ineqs:
        ...
        if __debug__:
            ...
            with Solver(env=env) as solver:
                solver.add_assertion(mgr.Not(mgr.Iff(curr, ineq.pysmt_ineq())))
                assert solver.solve() is False, (curr, ineq,
                                                 solver.get_model())
        ineq.to_real()
        ...
        if ineq.is_lt():
            ...
            log("\t    {}  <{}>  {}".format(symb, ineq_type, ineq), _LOG_LVL)
        elif ineq.is_le():
    ...
    assert all(len(k.get_free_variables() & params) == 0 for k in keys)
    ...
    if not lts:
        # le_const + eq_const < 0
        yield simplify(mgr.LT(const_constr, zero))
    else:
        # le_const + eq_const + lt_const <= 0
        yield simplify(mgr.LE(const_constr, zero))
        ...
        yield simplify(mgr.Or(disjunction))
