from typing import (Tuple, List, Set, FrozenSet, Iterator, Optional,
                    Union)
...
from expr_utils import fnode_key
...
_MOTZKIN_LOG_LVL = 1
_MOTZKIN_TIMEOUT = 25
_MOTZKIN_APPROX = True


def get_timeout() -> int:
    return _MOTZKIN_TIMEOUT


def set_timeout(val: int) -> None:
    assert isinstance(val, int)
    assert val > 0
    global _MOTZKIN_TIMEOUT
    _MOTZKIN_TIMEOUT = val


def get_motzkin_approximate() -> bool:
    return _MOTZKIN_APPROX


def set_motzkin_approximate(val: bool) -> None:
    assert isinstance(val, bool)
    global _MOTZKIN_APPROX
    _MOTZKIN_APPROX = val


def get_log_lvl() -> int:
    return _MOTZKIN_LOG_LVL


def set_log_lvl(val: int) -> None:
    assert isinstance(val, int)
    global _MOTZKIN_LOG_LVL
    _MOTZKIN_LOG_LVL = val


def motzkin_solve(env: PysmtEnv, e_symbs: Union[Set[FNode], FrozenSet[FNode]],
                  dnf: Iterator[Iterator[FNode]],
                  td: TimesDistributor,
                  pref_whole: Optional[FNode] = None):
    """Find assignment to e_symbs such that dnf is unsat"""
    assert isinstance(env, PysmtEnv)
    assert isinstance(e_symbs, (set, frozenset))
    assert all(s in env.formula_manager.get_all_symbols() for s in e_symbs)
    assert all(s.symbol_type().is_int_type() or s.symbol_type().is_real_type()
               for s in e_symbs)
    assert isinstance(td, TimesDistributor)
    assert td.env == env
    assert pref_whole is None or pref_whole in e_symbs

    with MultiSolver(env, get_timeout(), log_lvl=get_log_lvl()) as solver:
        for conj in dnf:
            solver.add_assertions(motzkin_transpose(env, conj, td, e_symbs))
        try:
            res = solver.solve()
        except SolverReturnedUnknownResultError:
            log("\t\tMotzkin timeout", get_log_lvl())
            res = None

        if res is True:
            if get_motzkin_approximate():
                # solver is unusable after this call.
                model = RationalApprox().simpl_model(env, solver, e_symbs,
                                                     pref_whole)
            return model
    return res


def motzkin_transpose(env: PysmtEnv, ineqs: Iterator[FNode],
                      td: TimesDistributor,
                      params: Union[Set[FNode],
                                    FrozenSet[FNode]]) -> Iterator[FNode]:
    ...
    log("\t\tMotzkin coefficients <type> ineqs: ", get_log_lvl() + 2)
    for curr in ineqs:
        assert isinstance(curr, FNode)
        assert curr in mgr.formulae.values()
        assert curr.is_lt() or curr.is_le() or curr.is_equals()
        if get_free_vars(curr) <= params:
            assert not simpl(curr).is_bool_constant()
            yield simpl(curr)
        else:
            ...
            log(f"\t\t    {serialize(symb)}  <{ineq_type}>  {ineq}",
                get_log_lvl() + 2)

    if len(eqs) + len(les) + len(lts) == 0:
        return

    # rewrite ToReal(s) as s.
    keys = list(sorted(frozenset(map(
        lambda k: k if not k.is_toreal() else k.arg(0),
        chain.from_iterable(ineq.lhs.keys() for ineq in chain(les, lts, eqs)))),
                       key=fnode_key))

    assert keys == list(sorted(frozenset(map(
        lambda k: k if not k.is_toreal() else k.arg(0),
    ...
    assert all(not p.is_bool_constant()
               for p in map(lambda x: mgr.GE(x, r_0
    ...
    for i in range(len(keys)):
        ...
        assert not simpl(mat_constr).is_bool_constant()
    ...
    if not lts:
        # le_const + eq_const < 0
        assert not simpl(mgr.LT(const_constr, zero)).is_bool_constant()
        yield simpl(mgr.LT(const_constr, zero))
    else:
        # le_const + eq_const + lt_const <= 0
        # assert not simpl(mgr.LE(const_constr, zero)).is_bool_constant()
        ...
        assert not simpl(mgr.Or(disjunction)).is_bool_constant()
