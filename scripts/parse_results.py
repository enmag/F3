...
def check_ctav(stdout, stderr, expected_res) -> str:
    ...
    error = re.compile(r"(\s+error\s+)|(exception)|(Exception)|(cannot open)|(syntax)|(segfault)",
                       re.IGNORECASE)
    res = None
    with open(stdout, "r") as in_f:
        for line in in_f:
...
def check_nuxmv(stdout, stderr, expected_res) -> str:
    assert os.path.isfile(stdout)
    assert os.path.isfile(stderr)
    ...
    if expected_res is True:
        wrong = re.compile(r"-- (LTL )?specification\s+.*\s+is\s+false")
        correct = re.compile(r"-- (LTL )?specification\s+.*\s+is\s+true")
    else:
    ...
    if res in {"error", "wrong"}:
        if label not in known_errors["nuxmv"]:
            print(f"Found nuxmv {res.upper()} in {stdout}")
            res = "error"
        else:
            res = "unknown"
...
def check_f3(stdout, stderr, expected_res) -> str:
    ...
    if res in {"segfault", "error", "wrong"}:
        if label not in known_errors["f3"]:
...
def check_uppaal(stdout, stderr, expected_res) -> str:
    ...
    if res in {"error", "wrong"}:
        if label not in known_errors["uppaal"]:
            print(f"Found Ultimate {res.upper()} in {stdout}")
            res = "error"
        else:
            res = "unknown"
