...
class Atmoc(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        any_transm = " | ".join(f"s{i}.l = transm" for i in range(proc_num))
...
class Ctav(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        any_transm = r" \/ ".join(f"S{i}.transm" for i in range(proc_num))
        return template.format(any_transm=any_transm)
...
class Divine(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        defines = "\n".join(f"#define s{i}_t (S{i}.transm)"
                            for i in range(proc_num))
        any_transm = " || ".join(f"s{i}_t" for i in range(proc_num))
        return template.format(defines=defines, any_transm=any_transm)
...
class Ltsmin(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        any_transm = " || ".join(f"S{i}_is_t == 1" for i in range(proc_num))
        return template.format(any_transm=any_transm)
...
class Uppaal(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        any_transm = " or ".join(f"S{i}.transm" for i in range(proc_num))
        return template.format(any_transm=any_transm)
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "tltl_csma")
