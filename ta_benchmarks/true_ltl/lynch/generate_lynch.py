...
class Atmoc(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        all_proc_stutter = " & ".join(f"p{i}.evt_stutter"
                                      for i in range(proc_num))
        spec = " & ".join(f"((FALSE SR (TRUE US p{i}.l = CS7)) -> (FALSE SR (p{i}.l != CS7 US v1 = {i+1})))"
                          for i in range(proc_num))
...
class Ctav(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        spec = r" /\ ".join(f'(([] <> P{i}.CS7) -> [] ((! P{i}.CS7) U "v1 == {i+1}"))'
                            for i in range(proc_num))
...
class Divine(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        defines = "\n".join(f"#define p{i}_cs (P{i}.CS7)\n"
                            f"#define veq{i+1} (v1 == {i+1})"
                            for i in range(proc_num))
        spec = " && ".join(f"(([] <> p{i}_cs) -> [] ((! p{i}_cs) U veq{i+1}))"
                           for i in range(proc_num))
...
class Ltsmin(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        spec = " && ".join(f"(([] <> P{i}_is_cs == 1) -> [] ((P{i}_is_cs == 0) U (v1 == {i+1})))"
                           for i in range(proc_num))
        return template.format(spec=spec)
...
class NuXmv(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        all_proc_stutter = " & ".join(f"p{i}.evt_stutter"
                                      for i in range(proc_num))
        spec = " & ".join(f"((G F p{i}.l = CS7) -> G (p{i}.l != CS7 U v1 = {i+1}))"
                          for i in range(proc_num))
        return template.format(proc_num_p=proc_num+1,
                               proc_decls=proc_decls,
                               all_proc_stutter=all_proc_stutter,
...
class Uppaal(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        spec = " and ".join(f"((A[] A<> P{i}.CS7) -> A[] (! P{i}.CS7 U v1 = {i+1}))"
                            for i in range(proc_num))
        return template.format(spec=spec)
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "tltl_lynch_protocol")
