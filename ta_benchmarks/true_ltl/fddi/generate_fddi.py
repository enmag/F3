...
class Atmoc(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        all_f_idle = " & ".join(f"(FALSE SR (TRUE SU (st{i}.l = y_idle)))"
                                for i in range(proc_num))
...
class Ctav(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        all_f_idle = r" /\ ".join(f"([] <> S{i}.station_y_idle)"
                                  for i in range(proc_num))
        return template.format(all_f_idle=all_f_idle)
...
class Divine(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        defines = "\n".join(f"#define s{i}_y_idle (S{i}.station_y_idle)"
                            for i in range(proc_num))
        all_f_idle = " && ".join(f"(G F s{i}_y_idle)" for i in range(proc_num))
        return template.format(defines=defines, all_f_idle=all_f_idle)
...
class Ltsmin(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        all_f_idle = " && ".join(f"([] <> S{i}_is_y_idle == 1)"
                                 for i in range(proc_num))
...
class NuXmv(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        all_f_idle = " & ".join(f"(G F st{i}.l = y_idle)"
                                for i in range(proc_num))
...
def main(num_models, bench_label):
    ...
    tools = [# Atmoc(base_dir),  # can only falsify.
             ...
             Ltsmin(base_dir),
             NuXmv(base_dir),
             # Uppaal(base_dir)  # not in spec language.
    ]
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "tltl_fddi")
