...
class Atmoc(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        all_req = " & ".join(f"P{i}.location = req" for i in range(proc_num))
...
class Ctav(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        all_req = r" /\ ".join(f"P{i}.req" for i in range(proc_num))
        return template.format(all_req=all_req)
...
class Divine(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        defines = "\n".join(f"#define p{i}_req (P{i}.req)"
                            for i in range(proc_num))
        all_req = " && ".join(f"p{i}_req" for i in range(proc_num))
        return template.format(defines=defines, all_req=all_req)
...
class Ltsmin(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        all_req = " && ".join(f"P{i}_is_req == 1" for i in range(proc_num))
        return template.format(all_req=all_req)
...
class Uppaal(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        all_req = " and ".join(f"P{i}.req" for i in range(proc_num))
        return template.format(all_req=all_req)
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "finv_fischer")
