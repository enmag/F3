...
class Atmoc(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        all_proc_stutter = " & ".join(f"p{i}.evt_stutter"
                                      for i in range(proc_num))
        all_l3 = " & ".join(f"p{i}.l = L3" for i in range(proc_num))
...
class Ctav(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        all_l3 = " && ".join(f"P{i}.L3" for i in range(proc_num))
        return template.format(all_l3=all_l3)
...
class Divine(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        comps_decl = "\n".join(f"  P{i} = Proc({i});" for i in range(proc_num))
    ...
    def _instantiate_property(self, template, proc_num):
        defines = "\n".join(f"#define p{i}_l3 (P{i}.L3)"
                            for i in range(proc_num))
        all_l3 = " && ".join(f"p{i}_l3" for i in range(proc_num))
...
class Uppaal(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        all_l3 = " and ".join(f"P{i}.L3" for i in range(proc_num))
        return template.format(all_l3=all_l3)
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "finv_lynch_protocol")
