...
class Ctav(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        spec = r" /\ ".join(fr"(S{i}.z_sync \/  S{i}.z_async \/ "
                            fr"S{i}.y_sync \/ S{i}.y_async)"
                            for i in range(proc_num))
...
class Divine(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        defines = "\n".join(f"#define s{i}_z_sync (S{i}.z_sync)\n"
                            f"#define s{i}_z_async (S{i}.z_async)\n"
                            f"#define s{i}_y_sync (S{i}.y_sync)\n"
                            f"#define s{i}_y_async (S{i}.y_async)"
                            for i in range(proc_num))
        spec = r" && ".join(f"(s{i}_z_sync ||  s{i}_z_async || "
                            f"s{i}_y_sync || s{i}_y_async)"
                            for i in range(proc_num))
...
class Ltsmin(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        spec = r" && ".join(f"(S{i}_loc == 1 ||  S{i}_loc == 2 || "
                            f"S{i}_loc == 3 || S{i}_loc == 4)"
                            for i in range(proc_num))
        return template.format(spec=spec)
...
class Uppaal(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        spec = " and ".join(f"(S{i}.z_sync or S{i}.z_async or "
                            f"S{i}.y_sync or S{i}.y_async)"
                            for i in range(proc_num))
        return template.format(spec=spec)
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "tinv_fddi")
