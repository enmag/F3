...
class Atmoc(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        proc_decls = "\n".join(f"  p{i} : P({i + 1}, v1, v2, _T);"
                               for i in range(proc_num))
        all_proc_stutter = " & ".join(f"p{i}.evt_stutter"
                                      for i in range(proc_num))
        mut_excl = []
        for i0 in range(proc_num):
            curr = " | ".join(f"p{i1}.l = CS7" for i1 in range(proc_num)
                              if i1 != i0)
            curr = f"(p{i0}.l = CS7 -> !({curr}))"
            mut_excl.append(curr)
        return template.format(proc_num_p=proc_num+1,
                               proc_decls=proc_decls,
                               all_proc_stutter=all_proc_stutter,
                               mut_excl=" & ".join(mut_excl))
...
class Ctav(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        proc_decls = "\n".join(f"  P{i} = P({i+1});"
                               for i in range(proc_num))
        proc_list = ", ".join(f"P{i}" for i in range(proc_num))
    ...
    def _instantiate_property(self, template, proc_num):
        mut_excl = []
        for i0 in range(proc_num):
            curr = r" \/ ".join(f"P{i1}.CS7" for i1 in range(proc_num)
                                if i1 != i0)
            curr = f"(P{i0}.CS7 -> ! ({curr}))"
            mut_excl.append(curr)
...
class Divine(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        comps_decl = "\n".join(f"  P{i} = Proc({i+1});" for i in range(proc_num))
    ...
    def _instantiate_property(self, template, proc_num):
        defines = "\n".join(f"#define p{i}_cs (P{i}.CS7)"
                            for i in range(proc_num))
        mut_excl = []
        for i0 in range(proc_num):
            curr = r" && ".join(f"p{i1}_cs" for i1 in range(proc_num)
                                if i1 != i0)
...
class Ltsmin(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        mut_excl = []
        for i0 in range(proc_num):
            curr = r" || ".join(f"P{i1}_is_cs == 1" for i1 in range(proc_num)
                                if i1 != i0)
            curr = f"(P{i0}_is_cs == 1 -> ! ({curr}))"
            mut_excl.append(curr)
        return template.format(mut_excl=" && ".join(mut_excl))
...
class Uppaal(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        mut_excl = []
        for i0 in range(proc_num):
            curr = r" or ".join(f"P{i1}.CS7" for i1 in range(proc_num)
                                if i1 != i0)
            curr = f"(P{i0}.CS7 imply not ({curr}))"
            mut_excl.append(curr)
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "tinv_lynch_protocol")
