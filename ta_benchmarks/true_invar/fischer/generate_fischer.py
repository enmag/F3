...
class Atmoc(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        for i0 in range(proc_num):
            curr = " | ".join(f"P{i1}.l = cs" for i1 in range(proc_num)
                              if i1 != i0)
            curr = f"(P{i0}.l = cs -> ! ({curr}))"
...
class Uppaal(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        mut_excl = []
        for i0 in range(proc_num):
            curr = r" or ".join(f"P{i1}.cs" for i1 in range(proc_num)
                                if i1 != i0)
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "tinv_fischer")
