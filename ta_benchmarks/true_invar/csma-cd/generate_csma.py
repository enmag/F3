...
class Atmoc(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        any_transm = " | ".join(f"s{i}.l = transm" for i in range(1, proc_num))
...
class Ctav(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        any_transm = r" \/ ".join(f"S{i}.transm" for i in range(1, proc_num))
...
class Ltsmin(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        any_transm = " || ".join(f"S{i}_is_transm == 1"
                                 for i in range(1, proc_num))
        return template.format(any_transm=any_transm)
...
class Uppaal(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        any_transm = " or ".join(f"S{i}.transm" for i in range(1, proc_num))
        return template.format(any_transm=any_transm)
...
def main(num_models, bench_label):
    ...
    tools = [Atmoc(base_dir),
             Ctav(base_dir),
             Divine(base_dir),
             # Ltsmin(base_dir),  # does not support clock in spec.
             NuXmv(base_dir),
             Uppaal(base_dir)]
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "tinv_csma")
