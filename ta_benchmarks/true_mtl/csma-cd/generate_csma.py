...
class Ctav(Tool):
    ...
    def _instantiate_property(self, template, proc_num):
        other_transm = " \/ ".join(f"S{i}.transm" for i in range(1, proc_num))
        return template.format(other_transm=other_transm)
...
class NuXmv(Tool):
    ...
    def _instantiate_model(self, template, proc_num):
        ...
        cd_events = " & ".join(
            f"((bus.evt_cd & bus.cd_id = {i}) <-> s{i}.evt_cd)"
            for i in range(proc_num))
        other_transm = " | ".join(f"s{i}.l = transm"
...
if __name__ == "__main__":
    main(config_args().parse_args().num, "tmtl_csma")
